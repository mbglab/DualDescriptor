...
class HierDDLrnC(nn.Module):
    ...
    def predict_l(self, seq, threshold=0.5):
        ...
        with torch.no_grad():
            ...
            logits = self.labeller(seq_vector.unsqueeze(0))
    ...
    def cls_train(self, seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        Adapted for HierDDLrnC architecture with variable-length sequence support.
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                for seq in batch_seqs:
                    ...
                    output = self.forward(seq)  # [1, final_seq_len, final_dim]
                    seq_vector = output.mean(dim=1)  # Average over sequence length [1, final_dim]
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                if batch_predictions_list:
                    batch_logits = torch.cat(batch_predictions_list, dim=0)
    ...
    def reconstruct(self, L, tau=0.0):
        ...
        Args:
            L (int): Length of sequence to reconstruct (must be <= max_input_seq_len)
        ...
        # Reconstruct sequence token by token
        while len(reconstructed_sequence) < L:
            candidate_scores = {}
            
            # Evaluate each possible token
            for token in self.tokens:
                candidate_seq = reconstructed_sequence + token
            ...
            if not candidate_scores:
                ...
                char_layer_rec = self.char_generate(L - len(reconstructed_sequence), tau)
                reconstructed_sequence += char_layer_rec
                break
            ...
            reconstructed_sequence += chosen_token
        
        # Ensure exact length and remove padding
        final_sequence = reconstructed_sequence[:L]
...
if __name__ == "__main__":
    ...
    cls_history = cls_model.cls_train(class_seqs, class_labels, num_classes, 
                                     max_iters=30, tol=1e-8, learning_rate=0.1,
                                     decay_rate=0.98, batch_size=8, print_every=5)
    ...
    accuracy = correct / 10
    print(f"Accuracy on 10 samples: {accuracy:.4f} ({correct}/10)")
    
    # === Multi-Label Classification Task Example ===
    print("\n" + "="*50)
    print("Multi-Label Classification Task with Variable-Length Sequences")
    print("="*50)
    ...
    print("\nCreating multi-label classification model...")
    ...
    char_rec_seq = model.char_reconstruct()
    print(f"Character-level reconstruction: {char_rec_seq[:100]}...")
