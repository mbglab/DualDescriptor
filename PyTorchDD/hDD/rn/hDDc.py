...
# Hierarchical Dual Descriptor (Random AB matrix form) and Character Sequence Input (HierDDrnC)
# Combines character-level processing and hierarchical vector processing
# This program is for the demonstration of methodology and not fully refined.
# Author: Bin-Guang Ma (assisted by DeepSeek); Date: 2025-10-11 ~ 2026-1-17
...
class Layer(nn.Module):
    """
    Single layer of Hierarchical Dual Descriptor with random AB matrix form
    Uses the random AB matrix architecture from HierDDrn with character sequence input
    """
    def __init__(self, in_dim, out_dim, basis_dim, use_residual, device='cuda'):
        """
        Initialize a hierarchical layer with random AB matrix form
        ...
        # Basis matrix: basis_dim x out_dim (learnable)
    ...
    def forward(self, x):
        """
        Forward pass through the layer with random AB matrix form
...
class HierDDrnC(nn.Module):
    """
    Hierarchical Dual Descriptor with Random AB Matrix and Character Sequence Input
    Combines character-level processing and hierarchical random AB matrix layers
    """
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, 
        ...
        """
        Initialize HierDDrnC model with embedded character-level processing and random AB matrix layers
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                for seq in batch_seqs:
                    ...
                    if not tokens:
                        ...
                        seq_vector = torch.zeros(self.model_dims[-1], device=self.device)
                    else:
                        # Get hierarchical model output
                        output = self.forward(seq)  # [1, seq_len, final_dim]
                        # Compute sequence representation: average over sequence length
                        seq_vector = output.mean(dim=1).squeeze(0)
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                for seq in batch_seqs:
                    ...
                    output = self.forward(seq)  # [1, seq_len, final_dim]
                    # Compute sequence representation: average over sequence length
                    seq_representation = output.mean(dim=1).squeeze(0)
    ...
    def predict_c(self, seq):
        ...
        # Squeeze dimension from (1, num_classes) to (num_classes,)
    ...
    def predict_l(self, seq, threshold=0.5):
        ...
        # Squeeze dimension from (1, num labels) to (num labels,)
    ...
    def reset_parameters(self):
        ...
        # Reset label heads if they exist
    ...
    def count_parameters(self):
        ...
        if self.classifier is not None:
            ...
            print(f"{'Classifier':<15} | {'classifier.weight':<25} | {num:<15} | {shape:<20}")
            ...
            if self.classifier.bias is not None:
                ...
                print(f"{'Classifier':<15} | {'classifier.bias':<25} | {num:<15} | {shape:<20}")
                classifier_params += num
                total_params += num
        ...
        if self.labeller is not None:
            ...
            print(f"{'Labeller':<15} | {'labeller.weight':<25} | {num:<15} | {shape:<20}")
            labeller_params += num
            total_params += num
            ...
            if self.labeller.bias is not None:
                ...
                print(f"{'Labeller':<15} | {'labeller.bias':<25} | {num:<15} | {shape:<20}")
                labeller_params += num
                total_params += num
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("HierDDrnC - Hierarchical Dual Descriptor with Random AB Matrix and Character Input")
    print("Random AB Matrix Model with Character Sequence Processing Demonstration")
    print("="*60)
    ...
    print(f"Using random AB matrix architecture with character input")
    ...
    reg_history = model.reg_train(
        ...
        print_every=5,
        batch_size=16,
        checkpoint_file='hierddrnc_gd_checkpoint.pth',
    ...
    self_history_gap = self_model.self_train(
        ...
        print_every=5,
        batch_size=256,
        checkpoint_file='hierddrnc_self_checkpoint.pth',
    ...
    # Save model
    model.save("hierddrnc_model.pth")
    
    # Load model
    loaded_model = HierDDrnC.load("hierddrnc_model.pth", device=device)
