...
# Modified to support hierarchical structure with multiple layers and varying dimensions - PyTorch GPU Accelerated Version
...
class HierDDrn(nn.Module):
    ...
    def train_model(self, seqs, t_list, max_iters=1000, tol=1e-8,
        ...
        Uses batch-like processing for efficiency
    ...
    def auto_train(self, seqs, max_iters=100, tol=1e-16, learning_rate=0.01, 
        ...
        if not continued:
            for layer in self.layers:
                ...
                if layer.use_residual == 'separate' and hasattr(layer, 'residual_proj') and layer.residual_proj is not None:
        ...
        for it in range(max_iters):
            ...
            for seq in seqs_tensor:
                ...
                else:  # 'gap' mode (original implementation)
                    # Forward pass
    ...
    def count_parameters(self):
        ...
        for l_idx, (in_dim, out_dim, basis_dim, layer) in enumerate(zip(
            in_dims, self.model_dims, self.basis_dims, self.layers)):
            ...
            print(f"  Layer {l_idx} (in_dim: {in_dim}, out_dim: {out_dim}, basis_dim: {basis_dim}):")
...
if __name__ == "__main__":
    ...
    model_dims = [16, 8] # Output dimensions for each layer
    basis_dims = [150, 150]  # Basis dimensions for each layer
    use_residual_list = ['separate', 'shared']  # Residual types for each layer
    ...
    # Create model with specified residual types
    ...
    # Load the model
    loaded_model = HierDDrn.load("test_model.pth", device=device)
    ...
    loaded_model = HierDDrn.load("auto_model.pth", device=device)
