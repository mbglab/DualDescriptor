...
# The Hierarchical Numeric Dual Descriptor (Mixed AB and PM Layers) with Linker matrices in PyTorch
...
class LayerAB(nn.Module):
    """
    A single layer of the Hierarchical Numeric Dual Descriptor with Linker (AB matrix form)
...
class LayerPM(nn.Module):
    """Single layer of Hierarchical Numeric Dual Descriptor (P Matrix form)"""
    def __init__(self, in_dim, out_dim, in_seq, out_seq, linker_trainable, use_residual, device):
        """
        Initialize a hierarchical layer (PM form)
        ...
        self.P = nn.Parameter(torch.randn(out_dim, out_dim, device=device) * 0.1)
        
        # Precompute periods matrix (fixed, not trainable)
        periods = torch.zeros(out_dim, out_dim, dtype=torch.float32, device=device)
        for i in range(out_dim):
    ...
    def sequence_transform(self, T):
        ...
        # Fix: Adjust dimensions to match Linker matrix
        return torch.matmul(T.permute(0, 2, 1), self.Linker).permute(0, 2, 1)
...
class HierDDLmx(nn.Module):
    ...
    Args:
        ...
        basis_dims (list): Basis dimensions for each layer (for AB layers)
        linker_dims (list): Output sequence lengths for each layer
        input_seq_len (int): Fixed input sequence length
        layer_types (list): Type of each layer ('ab' or 'pm')
    ...
    def __init__(self, input_dim=4, model_dims=[4], basis_dims=[50], 
                 linker_dims=[50], input_seq_len=100, layer_types=['ab'],
                 linker_trainable=False, use_residual=None, device='cuda'):
        super().__init__()
        ...
        for i, out_dim in enumerate(model_dims):
            ...
            if layer_type == 'ab':
                ...
                raise ValueError(f"Unknown layer type: {layer_type}. Must be 'ab' or 'pm'.")
    ...
    def count_parameters(self):
        ...
        for l_idx, layer in enumerate(self.layers):
            ...
            layer_type = self.layer_types[l_idx]
            out_seq = self.linker_dims[l_idx]
            
            print(f"  Layer {l_idx} (type: {layer_type}, in_dim: {in_dim}, out_dim: {out_dim}, in_seq: {in_seq}, out_seq: {out_seq}):")
            ...
            if hasattr(layer, 'use_residual') and layer.use_residual == 'separate':
                ...
                print(f"    Residual params: {residual_params} (Trainable: {residual_params if hasattr(layer, 'residual_linker') and layer.residual_linker.requires_grad else 0})")
...
if __name__ == "__main__":
    ...
    basis_dims = [150, 100, 50] # Basis dimensions for each layer (for AB layers)
    ...
    # Save model
    model_mixed.save("hier_ddl_mx_model.pth")
    
    # Load model
    loaded_model = HierDDLmx.load("hier_ddl_mx_model.pth", device=device)
    ...
    # Test Case 3: Example of using the new auto_train and generate methods
    print("\n" + "="*50)
    print("Example of using auto_train and generate methods")
    print("="*50)

    # Create a new model for auto-training
    auto_model = HierDDLmx(
        input_dim=input_dim,
        model_dims=[10, 20, 10],
        basis_dims=basis_dims,
        linker_dims=linker_dims,
        input_seq_len=input_seq_len,
        layer_types=['pm', 'ab', 'pm'],
        linker_trainable=[True, False, True],
        use_residual=['separate', 'shared', None],  # Different residual for each layer
        device=device
    )
    ...
    loaded_model = HierDDLmx.load("auto_model_mx.pth", device=device)
