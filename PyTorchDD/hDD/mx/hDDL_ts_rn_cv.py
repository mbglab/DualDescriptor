...
class LayerRN(nn.Module):
    """
    Single layer from HierDDrn (Random AB matrix form) with Linker matrix.
    ...
    def __init__(self, in_dim, out_dim, basis_dim, in_seq_len, out_seq_len, 
                 linker_trainable=True, use_residual='separate', device='cuda'):
        super().__init__()
        self.in_dim = in_dim
    ...
    def forward(self, x):
        """
        Forward pass for RN layer with Linker matrix.
        ...
        B_vectors = self.Bbasis[self.basis_indices]  # (in_seq_len, out_dim)
        B_vectors = B_vectors.unsqueeze(0).expand(x.size(0), -1, -1)  # (batch_size, in_seq_len, out_dim)
        
        # Compute scalars: dot product of normalized and B_vectors along last dimension
        scalars = torch.einsum('bsd,bsd->bs', normalized, B_vectors)  # (batch_size, in_seq_len)
        
        # Select coefficient vectors: (in_seq_len, out_dim) -> expanded to (batch_size, in_seq_len, out_dim)
        A_vectors = self.Acoeff[:, self.basis_indices].t()  # (in_seq_len, out_dim)
        A_vectors = A_vectors.unsqueeze(0).expand(x.size(0), -1, -1)  # (batch_size, in_seq_len, out_dim)
        
        # Compute new features: scalars * A_vectors
        new_features = scalars.unsqueeze(2) * A_vectors  # (batch_size, in_seq_len, out_dim)
        
        # Apply Linker matrix for sequence length transformation
        # (batch_size, in_seq_len, out_dim) x (in_seq_len, out_seq_len) -> (batch_size, out_dim, out_seq_len)
        linked_output = torch.matmul(new_features.permute(0, 2, 1), self.Linker)
        
        # Transpose to (batch_size, out_seq_len, out_dim)
        main_output = linked_output.permute(0, 2, 1)
        
        # Residual connection processing 
        if self.use_residual == 'separate':
            if self.residual_proj is not None:
                residual = self.residual_proj(residual)
            # Apply Linker to residual
            residual_linked = torch.matmul(residual.permute(0, 2, 1), self.Linker)
            residual_out = residual_linked.permute(0, 2, 1)
...
class LayerTS(nn.Module):
    """
    Single layer from HierDDts (Tensor form) with Linker matrix.
    ...
    Args:
        ...
        device (str): Device to run computations on ('cuda' or 'cpu')
    """
    def __init__(self, in_dim, out_dim, num_basis, in_seq_len, out_seq_len,
                 linker_trainable=True, use_residual='separate', device='cpu'):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
    ...
    def forward(self, x):
        """
        Forward pass for TS layer with Linker matrix.
        ...
        k = torch.arange(seq_len, device=self.device).view(-1, 1, 1, 1).float()
        ...
        # Apply Linker matrix for sequence length transformation
        # (batch_size, in_seq_len, out_dim) x (in_seq_len, out_seq_len) -> (batch_size, out_dim, out_seq_len)
        linked_output = torch.matmul(Nk.permute(0, 2, 1), self.Linker)
        ...
        if self.use_residual == 'separate':
            ...
            if self.in_dim == self.out_dim and self.use_residual is not None:
                residual_linked = torch.matmul(x.permute(0, 2, 1), self.Linker)
                residual_out = residual_linked.permute(0, 2, 1)
                out = main_output + residual_out
            else:
                out = main_output
...
class HierDDLmxC(nn.Module):
    """
    Hierarchical Dual Descriptor with Mixed Layers, Linker Matrices and Character Sequence Input
    Combines character-level processing with mixed TS and RN hierarchical layers with Linker matrices
    ...
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, 
                 ...
                 model_dims=[2], layer_types=['ts'], basis_list=[5], use_residual_list=None,
                 linker_dims=[50], linker_trainable_list=None,
                 char_linker_trainable=None, device='cuda'):
        """
        Initialize HierDDLmxC model with variable-length character input and Linker matrices
        ...
        Args:
            ...
            char_linker_trainable: Whether character layer Linker matrix is trainable (None=use linker_trainable_list[0])
        ...
        if len(basis_list) != self.num_layers:
        ...
        # Process linker_trainable_list parameter
        if linker_trainable_list is None:
            self.linker_trainable_list = [True] * self.num_layers
        elif isinstance(linker_trainable_list, list):
            if len(linker_trainable_list) != self.num_layers:
                raise ValueError("linker_trainable_list length must match number of layers")
            self.linker_trainable_list = linker_trainable_list
        else:
            raise TypeError("linker_trainable_list must be list or None")
        ...
        if char_linker_trainable is None:
            ...
            self.char_linker_trainable = self.linker_trainable_list[0] if isinstance(self.linker_trainable_list, list) and len(self.linker_trainable_list) > 0 else True
        else:
        ...
        if self.char_layer_type == 'ts':
            ...
            # RN character layer
            self.char_embedding = nn.Embedding(len(self.tokens), self.vec_dim)
            # Note: For RN character layer, we'll process tokens individually and then apply Linker
        ...
        # Hierarchical layers (mixed TS and RN with Linker matrices)
        ...
        for i, (out_dim, layer_type, basis_val, use_residual, linker_trainable) in enumerate(
            zip(model_dims, layer_types, basis_list, self.use_residual_list, self.linker_trainable_list)):
            
            # For first hierarchical layer, use fixed hier_input_seq_len; for subsequent layers, use previous linker_dim
            in_seq = self.hier_input_seq_len if i == 0 else linker_dims[i-1]
    ...
    def char_batch_compute(self, k_tensor, token_indices):
        ...
        if self.char_layer_type == 'ts':
            ...
            # For RN character layer, we use a simpler approach: just return embeddings
            # The actual RN transformation will be applied in the hierarchical layers
            output = x
    ...
    def char_sequence_to_tensor(self, seq):
        ...
        char_output = output_batch.unsqueeze(0)
    ...
    def _save_checkpoint(self, checkpoint_file, iteration, history, optimizer, scheduler, best_loss):
        ...
        checkpoint = {
            ...
            'config': {
                ...
                'linker_trainable_list': self.linker_trainable_list,
    ...
    def reset_parameters(self):
        """Reset all model parameters including Linker matrices"""
        ...
        # Reset hierarchical layer parameters including Linker matrices
        for layer in self.hierarchical_layers:
    ...
    def count_parameters(self):
        """Count and print learnable parameters in the model by layer and type including Linker matrices"""
        total_params = 0
        trainable_params = 0
        ...
        # Hierarchical layer parameters including Linker matrices
        for i, layer in enumerate(self.hierarchical_layers):
            ...
            for name, param in layer.named_parameters():
                if param.requires_grad:
                    ...
                    trainable_status = "T" if param.requires_grad else "F"
                    print(f"{layer_name:<15} | {name:<25} | {num:<15} | {shape:<20} [{trainable_status}]")
                    layer_params += num
                    if param.requires_grad:
                        layer_trainable += num
            ...
            print(f"{layer_name:<15} | {f'TOTAL ({layer_type})':<25} | {layer_params:<15} | (trainable: {layer_trainable})")
            print("-"*80)
    ...
    def save(self, filename):
        """Save model state to file including Linker matrices configuration"""
        # Convert mean_target to numpy for saving
        mean_target_np = self.mean_target.cpu().numpy() if self.mean_target is not None else None
    ...
    def load(cls, filename, device=None):
        """Load model from file including Linker matrices configuration"""
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("HierDDLmxC - Hierarchical Dual Descriptor with Mixed Layers, Linker Matrices and Character Input")
    ...
    print(f"Linker dimensions: {linker_dims}")
    ...
    print("\nCreating HierDDLmxC model with variable-length support...")
    model = HierDDLmxC(
        charset=charset,
        rank=rank,
        vec_dim=vec_dim,
        ...
        linker_trainable_list=[True, False, True],  # Mixed trainable settings
    ...
    auto_model = HierDDLmxC(
        ...
        basis_list=basis_list,
        linker_dims=linker_dims,
        linker_trainable_list=[False, False, False],
