...
# Hierarchical Dual Descriptor (mixed TS/RN layers) with Character Sequence Input (HierDDLmxC)
# Combines character-level processing with mixed TS/RN hierarchical layers with Linker matrices
...
class LayerRN(nn.Module):
    """
    Single layer from HierDDrn (Random AB matrix form) with Adaptive Linker matrix for Variable-Length Sequences.
    
    Args:
        in_dim (int): Input dimension
        out_dim (int): Output dimension
        basis_dim (int): Basis dimension for this layer
        max_in_seq_len (int): Maximum input sequence length in the dataset
        max_out_seq_len (int): Maximum output sequence length (max_in_seq_len // reduction_factor)
    ...
    """
    def __init__(self, in_dim, out_dim, basis_dim, max_in_seq_len, max_out_seq_len, 
                 linker_trainable=True, use_residual='separate', device='cuda', reduction_factor=2):
        super().__init__()
        self.in_dim = in_dim
        ...
        self.Linker = nn.Parameter(torch.Tensor(max_in_seq_len, max_out_seq_len))
        ...
        self.register_buffer('basis_indices', 
                            torch.tensor([i % basis_dim for i in range(max_in_seq_len)], device=device))
    ...
    def adaptive_sequence_transform(self, x, actual_in_seq_len):
        ...
        output = torch.matmul(x.transpose(1, 2), adaptive_linker).transpose(1, 2)
        
        return output, actual_out_seq_len
    
    def forward(self, x, actual_seq_len=None):
        """
        Forward pass for RN layer with Adaptive Linker matrix for variable-length sequences.
        
        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, in_seq_len, in_dim)
            actual_seq_len (int): Actual sequence length (if None, use x.shape[1])
        
        Returns:
            torch.Tensor: Output tensor of shape (batch_size, out_seq_len, out_dim)
            int: Actual output sequence length
        """
        ...
        normalized = self.norm(transformed)  # (batch_size, in_seq_len, out_dim)
        
        # Select basis vectors: (in_seq_len, out_dim) -> expanded to (batch_size, in_seq_len, out_dim)
        B_vectors = self.Bbasis[self.basis_indices[:actual_seq_len]]  # (actual_seq_len, out_dim)
        B_vectors = B_vectors.unsqueeze(0).expand(x.size(0), -1, -1)  # (batch_size, actual_seq_len, out_dim)
        
        # Compute scalars: dot product of normalized and B_vectors along last dimension
        scalars = torch.einsum('bsd,bsd->bs', normalized, B_vectors)  # (batch_size, actual_seq_len)
        
        # Select coefficient vectors: (actual_seq_len, out_dim) -> expanded to (batch_size, actual_seq_len, out_dim)
        A_vectors = self.Acoeff[:, self.basis_indices[:actual_seq_len]].t()  # (actual_seq_len, out_dim)
        A_vectors = A_vectors.unsqueeze(0).expand(x.size(0), -1, -1)  # (batch_size, actual_seq_len, out_dim)
        
        # Compute new features: scalars * A_vectors
        new_features = scalars.unsqueeze(2) * A_vectors  # (batch_size, actual_seq_len, out_dim)
        
        # Apply Adaptive Linker matrix for variable-length sequence transformation
        main_output, actual_out_seq_len = self.adaptive_sequence_transform(new_features, actual_seq_len)
       
        # Residual connection processing 
        if self.use_residual == 'separate':
            if self.residual_proj is not None:
                residual = self.residual_proj(residual)
            # Apply Adaptive Linker to residual
            residual_out, _ = self.adaptive_sequence_transform(residual, actual_seq_len)
...
class LayerTS(nn.Module):
    """
    Single layer from HierDDts (Tensor form) with Adaptive Linker matrix for Variable-Length Sequences.
    ...
    Args:
        ...
        reduction_factor (int): Factor by which to reduce sequence length (default: 2)
    """
    def __init__(self, in_dim, out_dim, num_basis, max_in_seq_len, max_out_seq_len,
                 linker_trainable=True, use_residual='separate', device='cpu', reduction_factor=2):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        ...
        # Adaptive Linker matrix for variable-length sequences
        # Size is based on maximum sequence lengths in the dataset
        self.Linker = nn.Parameter(torch.empty(max_in_seq_len, max_out_seq_len))
    ...
    def forward(self, x, actual_seq_len=None):
        """
        Forward pass for TS layer with Adaptive Linker matrix for variable-length sequences.
        ...
        # Expand tensors for broadcasting: positions (actual_seq_len, 1, 1, 1)
        k = torch.arange(actual_seq_len, device=self.device).view(-1, 1, 1, 1).float()
        
        # Compute basis functions: phi = cos(2π * k / period)
        # periods shape: (1, out_dim, out_dim, num_basis)
        periods = self.periods.unsqueeze(0)
        phi = torch.cos(2 * math.pi * k / periods)  # (actual_seq_len, out_dim, out_dim, num_basis)
        
        # Apply position-dependent transformation
        # x_trans: (batch_size, actual_seq_len, out_dim) -> (batch_size, actual_seq_len, 1, out_dim, 1)
        x_exp = x_trans[:, :actual_seq_len].unsqueeze(2).unsqueeze(4)
        
        # P: (out_dim, out_dim, num_basis) -> (1, 1, out_dim, out_dim, num_basis)
        P_exp = self.P.unsqueeze(0).unsqueeze(0)
        
        # Compute: P * x' * phi
        # Result shape: (batch_size, actual_seq_len, out_dim, out_dim, num_basis)
        product = P_exp * x_exp * phi
        
        # Sum over j (input dim) and g (basis index)
        Nk = torch.sum(product, dim=(3, 4))  # (batch_size, actual_seq_len, out_dim)
        
        # Apply Adaptive Linker matrix for variable-length sequence transformation
        main_output, actual_out_seq_len = self.adaptive_sequence_transform(Nk, actual_seq_len)
        ...
        if self.use_residual == 'separate':
            ...
            if self.in_dim == self.out_dim and self.use_residual is not None:
                residual_out, _ = self.adaptive_sequence_transform(x, actual_seq_len)
                out = main_output + residual_out
            else:
                out = main_output
...
class HierDDLmxC(nn.Module):
    """
    Hierarchical Dual Descriptor with Mixed Layers, Adaptive Linker Matrices and Character Sequence Input
    Combines character-level processing with mixed TS and RN hierarchical layers with Adaptive Linker matrices
    Modified to support variable-length character sequences with adaptive Linker matrices
    """
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, 
                 ...
                 linker_dims=[50], linker_trainable_list=None,
                 reduction_factors=None, device='cuda'):
        """
        Initialize HierDDLmxC model with embedded character-level processing and mixed hierarchical layers with Adaptive Linkers for variable-length sequences
        ...
        Args:
            ...
            linker_trainable_list: Whether Linker matrices are trainable for each layer
        ...
        num_layers = len(model_dims)
        if reduction_factors is None:
            self.reduction_factors = [2] * num_layers  # Default: reduce by factor of 2
        elif isinstance(reduction_factors, list):
            if len(reduction_factors) != num_layers:
        ...
        # Validate dimensions
        if len(layer_types) != len(model_dims):
            raise ValueError("layer_types length must match model_dims length")
        if len(basis_list) != len(model_dims):
            raise ValueError("basis_list length must match model_dims length")
        ...
        if self.char_layer_type == 'ts':
            ...
            self.char_rn_layer = LayerRN(
                ...
                max_in_seq_len=self.max_token_seq_len,  # Use maximum token sequence length
                max_out_seq_len=self.max_token_seq_len,  # Same output length for character layer
                linker_trainable=True,
                use_residual='separate',
                device=self.device,
                reduction_factor=1  # No reduction for character layer
            )
        ...
        # Hierarchical layers (mixed TS and RN with Adaptive Linker matrices for variable-length sequences)
        self.hierarchical_layers = nn.ModuleList()
        in_dim = vec_dim  # Input to first hierarchical layer is output from char layer
        max_in_seq_len = self.max_token_seq_len  # Maximum input sequence length for first hierarchical layer
        
        if use_residual_list is None:
            use_residual_list = ['separate'] * len(model_dims)
            
        if linker_trainable_list is None:
            linker_trainable_list = [True] * len(model_dims)
            
        for i, (out_dim, layer_type, basis_val, use_residual, linker_trainable) in enumerate(
            zip(model_dims, layer_types, basis_list, use_residual_list, linker_trainable_list)):
            
            max_out_seq_len = linker_dims[i]
            ...
            max_in_seq_len = max_out_seq_len  # Next layer input sequence length is current layer output sequence length
    ...
    def char_batch_compute(self, k_tensor, token_indices):
        ...
        if self.char_layer_type == 'ts':
            ...
            # RN character layer computation - process through RN layer with Adaptive Linker
            # Reshape for RN layer: [1, token_seq_len, vec_dim] (single sequence)
            batch_size = 1  # Single sequence processing
            x_seq = x.view(batch_size, len(token_indices), self.vec_dim)
            output_seq, _ = self.char_rn_layer(x_seq, actual_seq_len=len(token_indices))  # [1, token_seq_len, vec_dim]
            # Flatten back to [token_seq_len, vec_dim]
            output = output_seq.view(len(token_indices), self.vec_dim)
    ...
    def char_sequence_to_tensor(self, seq):
        ...
        if self.char_layer_type == 'ts':
            # For TS layer, output is [token_seq_len, vec_dim], add batch dimension
            output_reshaped = output_batch.unsqueeze(0)
        else:
            # For RN layer, output is already [token_seq_len, vec_dim], add batch dimension
            output_reshaped = output_batch.unsqueeze(0)
        
        return output_reshaped, actual_token_seq_len
    ...
    def forward(self, seq):
        ...
        # Pass through hierarchical layers with Adaptive Linker matrices for variable-length sequences
    ...
    def _save_checkpoint(self, checkpoint_file, iteration, history, optimizer, scheduler, best_loss):
        ...
        checkpoint = {
            ...
            'config': {
                ...
                'linker_trainable_list': [layer.linker_trainable for layer in self.hierarchical_layers],
    ...
    def reconstruct(self):
        """
        Reconstruct representative character sequence using the entire hierarchical model with Adaptive Linker matrices.
        
        This method leverages the complete model architecture including both the character layer
        and all hierarchical layers with Adaptive Linker matrices to reconstruct a sequence that best represents 
    ...
    def generate(self, L, tau=0.0):
        """
        Generate character sequence of length L using the entire hierarchical model with Adaptive Linker matrices.
    ...
    def reset_parameters(self):
        """Reset all model parameters including Adaptive Linker matrices"""
        ...
        if self.char_layer_type == 'ts':
            ...
            nn.init.uniform_(self.char_rn_layer.Linker, -0.1, 0.1)
            if self.char_rn_layer.residual_proj is not None:
                nn.init.uniform_(self.char_rn_layer.residual_proj.weight, -0.1, 0.1)
        
        # Reset hierarchical layer parameters including Adaptive Linker matrices
        for layer in self.hierarchical_layers:
    ...
    def count_parameters(self):
        """Count and print learnable parameters in the model by layer and type including Adaptive Linker matrices"""
        total_params = 0
        ...
        if self.char_layer_type == 'ts':
            ...
            # Character RN layer parameters including Adaptive Linker
        ...
        # Hierarchical layer parameters including Adaptive Linker matrices
    ...
    def save(self, filename):
        """Save model state to file including Adaptive Linker matrices configuration"""
        # Convert mean_target to numpy for saving
        mean_target_np = self.mean_target.cpu().numpy() if self.mean_target is not None else None
    ...
    def load(cls, filename, device=None):
        """Load model from file including Adaptive Linker matrices configuration"""
        ...
        model = cls(
            ...
            linker_trainable_list=config.get('linker_trainable_list', [True] * len(config['model_dims'])),
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("HierDDLmxC - Hierarchical Dual Descriptor with Mixed Layers, Adaptive Linker Matrices and Character Input")
    ...
    print("\nCreating HierDDLmxC model with Adaptive Linker matrices...")
    model = HierDDLmxC(
        charset=charset,
        rank=rank,
        vec_dim=vec_dim,
    ...
    gd_history = model.grad_train(
        ...
        checkpoint_file='hierddlmxc_var_gd_checkpoint.pth',
    ...
    # Sequence reconstruction with variable-length support
    print("\n" + "="*50)
    ...
    auto_history_gap = auto_model.auto_train(
        ...
        auto_mode='gap',
        print_every=2,
        checkpoint_file='hierddlmxc_var_auto_checkpoint.pth',
    ...
    # Model save and load test with variable-length support
    print("\n" + "="*50)
    print("Model Save/Load Test with Variable-Length Support")
    print("="*50)
    
    # Save model
    model.save("hierddlmxc_var_model.pth")
