...
# Hierarchical Dual Descriptor with Mixed AB/PM Layers and Character Sequence Input (HierDDmxC)
# Combines character-level processing with mixed AB and PM hierarchical layers
...
class HierDDmxC(nn.Module):
    """
    Hierarchical Dual Descriptor with Mixed AB/PM Layers and Character Sequence Input
    Combines character-level processing with mixed AB/PM hierarchical layers
    ...
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, 
                 ...
                 use_residual_list=None, layer_types=None, char_layer_type='pm',
                 char_basis_dim=100, char_use_residual=None, device='cuda'):
        super().__init__()
        ...
        for i, (out_dim, basis_dim, layer_type, use_residual) in enumerate(
            zip(model_dims, basis_dims, layer_types, use_residual_list)):
            
            if layer_type == 'ab':
                layer = LayerAB(in_dim, out_dim, basis_dim, use_residual, self.device)
            else:  # 'pm'
                layer = LayerPM(in_dim, out_dim, use_residual, self.device)
    ...
    def char_batch_compute_Nk(self, k_tensor, token_indices):
        ...
        if self.char_layer_type == 'pm':
            ...
            if self.char_use_residual == 'separate':
                ...
                # For PM, shared residual would use the same transformation as main path
                # This is handled in the main PM layer implementation
                pass
                
        else:  # AB character layer
            batch_size = x.shape[0]
            seq_len = batch_size  # Each position is a separate "sequence" of length 1
            ...
            if self.char_use_residual == 'separate':
                ...
                residual = x  # Identity connection (in_dim == out_dim for char layer)
            ...
            scalars = torch.sum(transformed * B_vectors, dim=1)  # (batch_size,)
            
            # Select coefficient vectors: (batch_size, vec_dim)
            A_vectors = self.char_Acoeff[:, j_indices].t()  # (batch_size, vec_dim)
            
            # Compute new features: scalars * A_vectors
            Nk = scalars.unsqueeze(1) * A_vectors  # (batch_size, vec_dim)
    ...
    def forward(self, seq):
        ...
        for i, layer in enumerate(self.hierarchical_layers):
            if self.layer_types[i] == 'ab':
                x = layer(x)  # AB layers don't use positions
            else:  # PM layers
                x = layer(x, positions)
    ...
    def count_parameters(self):
        ...
        for i, layer in enumerate(self.hierarchical_layers):
            ...
            if layer_type == 'ab':
                ...
                if layer.use_residual == 'separate' and layer.residual_proj:
                    print(f"{layer_name:<15} | {'residual_proj':<25} | {residual_params:<15} | {tuple(layer.residual_proj.weight.shape)}")
            else:  # PM layer
                # PM layer parameters
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("HierDDmxC - Hierarchical Dual Descriptor with Mixed AB/PM Layers and Character Input")
    print("Mixed AB/PM Architecture with Character Sequence Processing")
    print("="*60)
    ...
    # Create model with PM character layer
    print("\nCreating HierDDmxC model with PM character layer...")
    model_pm = HierDDmxC(
        charset=charset,
        rank=rank,
        vec_dim=vec_dim,
    ...
    # Count parameters
    print("\nModel Parameter Count (PM Char Layer):")
    total_params_pm = model_pm.count_parameters()
    
    # Gradient descent training
    print("\n" + "="*50)
    print("Gradient Descent Training (PM Char Layer)")
    print("="*50)
    ...
    gd_history_pm = model_pm.grad_train(
        ...
        print_every=5,
        batch_size=32,
        checkpoint_file='gdmx_pm_checkpoint.pth',
        checkpoint_interval=10
    )
    
    # Predictions and correlation analysis
    print("\n" + "="*50)
    print("Prediction and Correlation Analysis (PM Char Layer)")
    print("="*50)
    
    pred_t_list_pm = [model_pm.predict_t(seq) for seq in seqs]
    ...
    print("\n" + "="*50)
    print("Sequence Reconstruction (PM Char Layer)")
    print("="*50)
    ...
    # Sequence generation
    print("\n" + "="*50)
    print("Sequence Generation (PM Char Layer)")
    print("="*50)
    
    # Deterministic generation
    det_seq_pm = model_pm.generate(L=100, tau=0.0)
    print(f"Deterministic generation (tau=0.0): {det_seq_pm}")
    
    # Stochastic generation
    stoch_seq_pm = model_pm.generate(L=100, tau=0.5)
    print(f"Stochastic generation (tau=0.5): {stoch_seq_pm}")
    
    # Create model with AB character layer
    print("\n" + "="*50)
    print("Testing AB Character Layer")
    print("="*50)
    ...
    print("\nModel Parameter Count (AB Char Layer):")
    ...
    auto_history = model_pm.auto_train(
        ...
        auto_mode='gap',
        print_every=5,
        checkpoint_file='automx_checkpoint.pth',
    ...
    # Save model
    model_pm.save("hierddmxc_model.pth")
    ...
    print("\n" + "="*60)
    print("HierDDmxC Demonstration Completed")
    print("="*60)
