...
class HierDDLmxC(nn.Module):
    ...
    Assumes fixed-length character sequences for consistent hierarchical processing
    """
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, 
                 ...
                 input_seq_len=100, device='cuda'):
        """
        Initialize HierDDLmxC model with embedded character-level processing and mixed hierarchical layers with Linkers
        ...
        Args:
            ...
            input_seq_len: Fixed input character sequence length for consistent processing
        ...
        if self.char_layer_type == 'ts':
            ...
            self.char_rn_layer = LayerRN(
                ...
                in_seq_len=self.token_seq_len,  # Use calculated token sequence length
                out_seq_len=self.token_seq_len,  # Same output length for character layer
    ...
    def char_batch_compute(self, k_tensor, token_indices):
        ...
        if self.char_layer_type == 'ts':
            ...
            # RN character layer computation - process through RN layer with Linker
            # Reshape for RN layer: [1, token_seq_len, vec_dim] (single sequence)
            batch_size = 1  # Single sequence processing
            x_seq = x.view(batch_size, self.token_seq_len, self.vec_dim)
            output_seq = self.char_rn_layer(x_seq)  # [1, token_seq_len, vec_dim]
            # Flatten back to [token_seq_len, vec_dim]
            output = output_seq.view(self.token_seq_len, self.vec_dim)
    ...
    def count_parameters(self):
        ...
        if self.char_layer_type == 'ts':
            ...
            # Character RN layer parameters including Linker
...
if __name__ == "__main__":
    ...
    print("Combined Model with Linker Matrices Demonstration")
    print("="*70)
    ...
    gd_history = model.grad_train(
        ...
        print_every=5,
        batch_size=32,
        checkpoint_file='hierddlmxc_gd_checkpoint.pth',
    ...
    auto_history_gap = auto_model.auto_train(
        ...
        auto_mode='gap',
        print_every=5,
        checkpoint_file='hierddlmxc_auto_checkpoint.pth',
    ...
    # Save model
    model.save("hierddlmxc_model.pth")
    
    # Load model
    loaded_model = HierDDLmxC.load("hierddlmxc_model.pth", device=device)
