...
# Author: Bin-Guang Ma (assisted by DeepSeek); Date: 2025-10-10 ~ 2026-1-18
...
class HierDDLtsC(nn.Module):
    ...
    Combines character-level processing and hierarchical vector processing with Linker matrices
    Assumes fixed-length character sequences as input
    """
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, num_basis=5, 
                 mode='linear', user_step=None, input_seq_len=100, model_dims=[2], 
        ...
        # Classification and multi-label heads (initialized later when needed)
        self.num_classes = None  # Number of classes for multi-class prediction
        self.classifier = None   # Linear layer for classification
        self.num_labels = None   # Number of labels for multi-label prediction
        self.labeller = None     # Linear layer for multi-label prediction
        ...
        for l in range(self.num_layers):
            ...
            in_seq = self.token_seq_len if l == 0 else linker_dims[l-1]  # Use token_seq_len instead of input_seq_len
    ...
    def batch_token_indices(self, seqs, device=None):
        """
        Vectorized batch conversion of character sequences to token indices
        ...
        L = self.input_seq_len  # Fixed input sequence length
    ...
    def batch_sequence_to_tensor(self, seqs):
        ...
        token_indices = self.batch_token_indices(seqs, device=self.device)
        
        # Compute char layer outputs
    ...
    def forward(self, input_data):
        ...
        if isinstance(input_data, str):
            ...
            x = self.char_forward(input_data.to(self.device))
        else:
            raise TypeError("Input must be str, list of strings, or Tensor")
    ...
    def extract_features(self, input_data):
        """
        Extract feature vectors from input sequences by averaging over sequence length
        
        Args:
            input_data (str or list or Tensor): Input character sequence(s) or token indices
            
        Returns:
            Tensor: Feature vectors of shape [batch_size, final_dim]
        """
        output = self.forward(input_data)  # [batch_size, final_seq_len, final_dim]
        features = output.mean(dim=1)      # Average over sequence length
        return features
    ...
    def reg_train(self, seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        # Pre-process data for efficient batch training
        ...
        for it in range(start_iter, max_iters):
            ...
            for i in range(0, num_samples, batch_size):
                ...
                batch_t = all_targets[batch_idx].to(self.device)
                
                optimizer.zero_grad()
                
                # Forward pass for the whole batch
                output = self.forward(batch_indices)  # [B, final_seq_len, final_dim]
                ...
                normalized_loss = loss / len(batch_idx)
    ...
    def cls_train(self, seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        Optimized for GPU memory efficiency with batch processing.
        ...
        for it in range(start_iter, max_iters):
            ...
            for batch_start in range(0, num_samples, batch_size):
                ...
                # Extract features by averaging over sequence length
                features = output.mean(dim=1)  # [B, final_dim]
    ...
    def lbl_train(self, seqs, labels, num_labels, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        if continued and checkpoint_file and os.path.exists(checkpoint_file):
            checkpoint = torch.load(checkpoint_file, map_location=self.device, weights_only=False)
            self.load_state_dict(checkpoint['model_state_dict'])
            if self.labeller and 'labeller_state_dict' in checkpoint:
        ...
        for it in range(start_iter, max_iters):
            ...
            for batch_start in range(0, num_samples, batch_size):
                ...
                logits = self.labeller(features)  # [B, num_labels]
    ...
    def predict_c(self, seq):
        ...
        # Ensure sequence length matches input_seq_len
        if len(seq) != self.input_seq_len:
            # Pad or truncate if necessary
            if len(seq) < self.input_seq_len:
                seq = seq.ljust(self.input_seq_len, 'A')
            else:
                seq = seq[:self.input_seq_len]
        
        # Get sequence representation through the entire model
        with torch.no_grad():
            output = self.forward(seq)  # [1, final_seq_len, final_dim]
            features = output.mean(dim=1)  # [1, final_dim]
    ...
    def reset_parameters(self):
        ...
        # Reset classification and label heads if they exist
    ...
    def count_parameters(self):
        ...
        # Classification and label heads
        if self.classifier is not None:
            num = sum(p.numel() for p in self.classifier.parameters())
            shape = f"Linear({self.model_dims[-1] if self.model_dims else self.vec_dim}->{self.num_classes})"
        ...
        if self.labeller is not None:
            ...
            shape = f"Linear({self.model_dims[-1] if self.model_dims else self.vec_dim}->{self.num_labels})"
    ...
    def save(self, filename):
        ...
        # Add classification head information if exists
        if self.classifier is not None:
            config['num_classes'] = self.num_classes
        
        # Add label head information if exists
        if self.labeller is not None:
    ...
    def load(cls, filename, device=None):
        ...
        # First, get the model's current state dict
        model_state_dict = model.state_dict()
        
        # Then, get the saved state dict
        saved_state_dict = checkpoint['model_state_dict']
        
        # Filter out keys that don't exist in the current model
        # (This handles cases where classifier/labeller weights are saved but model doesn't have them yet)
        filtered_state_dict = {}
        for key, value in saved_state_dict.items():
            if key in model_state_dict:
                filtered_state_dict[key] = value
            else:
                print(f"Warning: Skipping key '{key}' as it doesn't exist in the current model")
        
        # Load the filtered state dict
        model.load_state_dict(filtered_state_dict, strict=False)
        
        # Load classification head if it exists in checkpoint
        ...
        # Load label head if it exists in checkpoint
        if 'labeller_state_dict' in checkpoint and checkpoint['labeller_state_dict'] is not None:
            ...
            if num_labels is not None:
                ...
                print(f"Loaded labeller with {num_labels} labels")
...
if __name__ == "__main__":
    ...
    # Test character-level processing
    print("\n" + "="*50)
    print("Character-Level Processing Test")
    print("="*50)
    ...
    # Test batch token indices method
    print("\nTesting batch_token_indices method...")
    batch_indices = model.batch_token_indices(seqs[:5])
    ...
    reg_history = model.reg_train(
        ...
        print_every=5,
        batch_size=8,
        checkpoint_file='hierddltsc_gd_checkpoint.pth',
    ...
    print("\n" + "="*50)
    print("Sequence Reconstruction (using reconstruct method)")
    print("="*50)
    ...
    print(f"\nFinal training loss: {lbl_loss_history[-1]:.6f}")
    print(f"Final training accuracy: {lbl_acc_history[-1]:.4f}")
    ...
    self_history_gap = self_model.self_train(
        seqs[:50],  # Use subset for faster training
        max_iters=20,
        learning_rate=0.01,
        self_mode='gap',
        print_every=5,
        batch_size=8,
        checkpoint_file='hierddltsc_self_checkpoint.pth',
    ...
    print(f"Original model prediction: {original_pred} (probs: {[f'{p:.4f}' for p in original_probs]})")
    print(f"Loaded model prediction:   {loaded_pred} (probs: {[f'{p:.4f}' for p in loaded_probs]})")
