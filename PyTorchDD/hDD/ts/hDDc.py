...
# Author: Bin-Guang Ma (assisted by DeepSeek); Date: 2025-10-8 ~ 2026-1-18
...
class HierDDtsC(nn.Module):
    """
    Hierarchical Dual Descriptor with Character Sequence Input
    Combines character-level processing (embedded from DualDescriptorTS) and hierarchical layers for vector processing
    """
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, num_basis=5, 
                 mode='linear', user_step=None, model_dims=[2], num_basis_list=[5], 
                 use_residual_list=None, device='cuda'):
        """
        Initialize HierDDtsC model with embedded character-level processing
        ...
        for out_dim, num_basis, use_residual in zip(model_dims, num_basis_list, use_residual_list):
            layer = Layer(in_dim, out_dim, num_basis, use_residual)
        ...
        # Label head for multi-label prediction
    ...
    def batch_sequence_to_tensor(self, seqs):
        ...
        Returns:
            Tensor: Sequence tensor of shape [batch_size, max_seq_len, vec_dim]
        """
        batch_tensors = []
        for s in seqs:
        ...
        # Stack all sequences
        return torch.stack(batch_tensors)  # [batch_size, seq_len, vec_dim]
    ...
    def forward(self, seq):
        ...
        Args:
            seq (str or list or Tensor): Input character sequence(s) or precomputed tensor
        ...
        if isinstance(seq, str):
            ...
            # Multiple sequences
            x = self.batch_sequence_to_tensor(seq)
    ...
    def cls_train(self, seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        Optimized for hierarchical model with batch processing.
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                for seq in batch_seqs:
                    ...
                    with torch.set_grad_enabled(True):
                        output = self.forward(seq)  # [1, seq_len, final_dim]
                        # Average over sequence length to get sequence representation
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                if batch_logits:
                    ...
                    batch_loss = criterion(batch_logits_tensor, batch_labels)
                    
                    # Calculate accuracy
                    with torch.no_grad():
                        # Apply sigmoid to get probabilities
                        probs = torch.sigmoid(batch_logits_tensor)
    ...
    def self_train(self, seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
        ...
        Args:
            ...
            decay_rate: Learning rate decay rate
            print_every: Print interval
            batch_size: Batch size for vectorized training
            checkpoint_file: Path to save/load checkpoint file for resuming training
            checkpoint_interval: Interval (in iterations) for saving checkpoints
        ...
        # Pre-process sequences to reduce CPU-GPU data transfer
        print("Pre-processing training data...")
        # Group sequences by length for efficient batch processing
        seq_groups = {}
        for seq in seqs:
            length = len(seq)
            if length not in seq_groups:
                seq_groups[length] = []
            seq_groups[length].append(seq)
        
        # Convert groups to lists for batch processing
        seq_batches = []
        for length, group_seqs in seq_groups.items():
            # Split large groups into smaller batches
            for i in range(0, len(group_seqs), batch_size):
                batch = group_seqs[i:i+batch_size]
        ...
        for it in range(start_iter, max_iters):
            ...
            random.shuffle(seq_batches)
            
            # Process each batch
            for batch_seqs in seq_batches:
                if not batch_seqs:
                    continue
                    
                optimizer.zero_grad()
                
                # Convert batch to tensor through char layer
                char_output = self.batch_sequence_to_tensor(batch_seqs)  # [batch_size, seq_len, vec_dim]
                batch_size, seq_len, _ = char_output.shape
                
                if seq_len <= 1 and self_mode == 'reg':
                    continue  # Skip batches that are too short for reg mode
                ...
                if self_mode == 'gap':
                    ...
                    # Compare hierarchical output with char output
                    diff = hierarchical_output - char_output
                    sq_diff = torch.sum(diff ** 2, dim=-1)  # [batch_size, seq_len]
                    batch_loss = torch.sum(sq_diff) / (batch_size * seq_len)
                    valid_positions = batch_size * seq_len
                else:  # 'reg' mode
                    # Predict next position's char layer output
                    if seq_len > 1:
                        targets = char_output[:, 1:, :]  # [batch_size, seq_len-1, vec_dim]
                        preds = hierarchical_output[:, :seq_len-1, :]  # [batch_size, seq_len-1, final_dim]
                        diff = preds - targets
                        sq_diff = torch.sum(diff ** 2, dim=-1)  # [batch_size, seq_len-1]
                        batch_loss = torch.sum(sq_diff) / (batch_size * (seq_len - 1))
                        valid_positions = batch_size * (seq_len - 1)
    ...
    def load(cls, filename, device=None):
        ...
        if 'classification_heads' in checkpoint:
            ...
            if model.num_classes is not None:
                ...
                model.classifier = nn.Linear(final_dim, model.num_classes).to(device)
                # Load classifier weights from state_dict
                for name, param in model.classifier.named_parameters():
                    if f'classifier.{name}' in model.state_dict():
                        param.data.copy_(model.state_dict()[f'classifier.{name}'])
            
            if model.num_labels is not None:
                final_dim = model.model_dims[-1] if model.model_dims else model.vec_dim
                model.labeller = nn.Linear(final_dim, model.num_labels).to(device)
                # Load labeller weights from state_dict
                for name, param in model.labeller.named_parameters():
                    if f'labeller.{name}' in model.state_dict():
                        param.data.copy_(model.state_dict()[f'labeller.{name}'])
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("HierDDtsC - Hierarchical Dual Descriptor with Character Input")
    ...
    correct = 0
    for i, (seq, true_label) in enumerate(zip(class_seqs[:5], class_labels[:5])):
    ...
    accuracy = correct / 5
    print(f"Accuracy on first 5 sequences: {accuracy:.2f}")
    ...
    for i, (seq, true_labels) in enumerate(zip(label_seqs[:3], multi_labels[:3])):
        binary_preds, probs = lbl_model.predict_l(seq, threshold=0.5)
        print(f"Seq {i+1}:")
    ...
    # Self-train in gap mode with batch processing
    print("\nSelf-training in 'gap' mode with batch processing...")
    ...
    # Save model
    model.save("hierddtsc_model.pth")
    
    # Load model
    loaded_model = HierDDtsC.load("hierddtsc_model.pth", device=device)
