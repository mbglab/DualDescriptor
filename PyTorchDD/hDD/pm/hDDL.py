...
class HierDDLpm(nn.Module):
    ...
    def cls_train(self, seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        for it in range(start_iter, max_iters):
            ...
            for i in range(0, num_samples, batch_size):
                ...
                # Get batch data
                batch_seqs = seqs_tensor[batch_idx]  # [batch_size, seq_len, input_dim]
                batch_labels = labels_tensor[batch_idx]  # [batch_size]
    ...
    def lbl_train(self, seqs, labels, num_labels, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        for it in range(start_iter, max_iters):
            ...
            for i in range(0, num_samples, batch_size):
                ...
                # Get batch data
                batch_seqs = seqs_tensor[batch_idx]  # [batch_size, seq_len, input_dim]
                batch_labels = labels_tensor[batch_idx]  # [batch_size, num_labels]
    ...
    def count_parameters(self):
        ...
        if self.classifier is not None:
            ...
            print(f"{'Classifier':<10} | {'classifier':<15} | {num:<15} | {shape:<20} | {'Yes':<10}")
            total_params += num
            
        if self.labeller is not None:
            num = sum(p.numel() for p in self.labeller.parameters())
            shape = f"({self.model_dims[-1]}, {self.num_labels})"
            print(f"{'Labeller':<10} | {'labeller':<15} | {num:<15} | {shape:<20} | {'Yes':<10}")
            total_params += num
...
if __name__ == "__main__":
    ...
    # Test Case 8: Multi-class Classification
    print("\n" + "="*50)
    print("=== Test Case 8: Multi-class Classification ===")
    print("="*50)
    ...
    # Test Case 9: Multi-label Classification
    print("\n" + "="*50)
    print("=== Test Case 9: Multi-label Classification ===")
    print("="*50)
    ...
    # Test Case 10: Combined model with both classification heads
    print("\n" + "="*50)
    print("=== Test Case 10: Combined Model ===")
    print("="*50)
    
    # Initialize model with both classification heads (though typically you'd use one at a time)
    model_combined = HierDDLpm(
        input_dim=input_dim,
        model_dims=[8, 6, 3],
        input_seq_len=input_seq_len,
        linker_dims=[80, 40, 20],
    ...
    total_params = model_combined.count_parameters()
