...
class HierDDpmC(nn.Module):
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                for seq in batch_seqs:
                    ...
                    if output.shape[1] > 0:  # Check if sequence has tokens
                        seq_vector = output.mean(dim=1).squeeze(0)
                    else:
                        # For empty sequences, use zero vector
                        final_dim = self.model_dims[-1] if self.model_dims else self.vec_dim
                        seq_vector = torch.zeros(final_dim, device=self.device)
    ...
    def predict_l(self, seq, threshold=0.5):
        ...
        with torch.no_grad():
            ...
            if output.shape[1] > 0:  # Check if sequence has tokens
                ...
                # For empty sequences, use zero vector and return zeros
    ...
    def load(cls, filename, device=None):
        ...
        if 'classification_heads' in checkpoint:
            ...
            if heads_info.get('num_classes') is not None:
                final_dim = config['model_dims'][-1] if config['model_dims'] else config['vec_dim']
                model.num_classes = heads_info['num_classes']
                model.classifier = nn.Linear(final_dim, model.num_classes).to(device)
                # Load classifier weights (if saved separately, but they should be in state_dict)
            if heads_info.get('num_labels') is not None:
                final_dim = config['model_dims'][-1] if config['model_dims'] else config['vec_dim']
                model.num_labels = heads_info['num_labels']
                model.labeller = nn.Linear(final_dim, model.num_labels).to(device)
                # Load labeller weights (if saved separately, but they should be in state_dict)
...
if __name__ == "__main__":
    ...
    correct = 0
    for seq, true_label in zip(class_seqs[:10], class_labels[:10]):
        pred_class, probs = cls_model.predict_c(seq)
        if pred_class == true_label:
            correct += 1
        print(f"Seq: {seq[:30]}... True: {true_label}, Pred: {pred_class}, Probs: {[f'{p:.3f}' for p in probs]}")
    
    print(f"Accuracy on 10 samples: {correct}/10 = {correct/10:.2f}")
    ...
    for _ in range(80):
        ...
        lbl_labels.append([1.0 if x else 0.0 for x in label_vec])
    ...
    loss_history, acc_history = lbl_model.lbl_train(
        lbl_seqs, lbl_labels, num_labels,
        max_iters=50,
        learning_rate=0.03,
        decay_rate=0.99,
        batch_size=16,
    ...
    for i in range(3):
        ...
        print(f"All correct:    {np.all(binary_pred == np.array(true_labels))}")
