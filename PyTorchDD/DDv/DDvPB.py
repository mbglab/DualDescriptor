...
# The Dual Descriptor Vector class (PB Matrix form) implemented with PyTorch
# This program is for the demonstration of methodology and not fully refined.
# Author: Bin-Guang Ma (assisted by DeepSeek); Date: 2025-7-29 ~ 2026-1-29
...
class DualDescriptorPB(nn.Module):
    """
    Vector Dual Descriptor with GPU acceleration using PyTorch:
      ...
      - Modified: Using trainable B matrix as position encoding
    """
    def __init__(self, charset, rank=1, rank_mode='drop', vec_dim=2, mode='linear', user_step=None, device='cuda', trainable_b=True):
        super().__init__()
        ...
        self.trainable_b = trainable_b  # Whether B matrix is trainable
        ...
        # MODIFIED: Trainable B matrix as position encoding
        # Create a B matrix with shape [m, m]
        # This matrix can be trainable or fixed based on trainable_b parameter
        if self.trainable_b:
            self.B = nn.Parameter(torch.empty(self.m, self.m))
        else:
            self.register_buffer('B', torch.empty(self.m, self.m))
    ...
    def reset_parameters(self):
        ...
        nn.init.uniform_(self.B, -0.1, 0.1)  # Initialize B matrix with uniform distribution
    ...
    def batch_compute_Nk(self, k_tensor, token_indices):
        ...
        # MODIFIED: Use trainable B matrix as position encoding
        # B: [m, m] (can be trainable or fixed based on trainable_b parameter)
        # The position index k is not used in the computation with B
        # This simplifies the computation to a trainable transformation
        
        # Optimized computation using einsum
        # P: [m, m], x: [batch_size, m], B: [m, m] (trainable or fixed)
        # The computation is now: N(k) = x * P * B (for each position)
        Nk = torch.einsum('bj,ij,jk->bi', x, self.P, self.B)
    ...
    def self_train(self, seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
        ...
        """
        Self-training method using self-consistency mode with memory-efficient sequence processing.
        Trains the model by minimizing the difference between N(k) vectors and corresponding token embeddings.
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                for seq in batch_seqs:
                    ...
                    # Self-consistency training: N(k) should match token embedding at position k
                    seq_loss = 0.0
                    valid_positions = len(tokens)
...
if __name__=="__main__":
    ...
    print("MODIFIED: Using trainable B matrix as position encoding")
    ...
    # Initialize the model with trainable_b=True (default)
    dd = DualDescriptorPB(
        charset, 
        rank=rank, 
        vec_dim=vec_dim, 
        ...
        trainable_b=True  # Default value, can be omitted
    )
    
    # Display device information
    print(f"\nUsing device: {dd.device}")
    print(f"Number of tokens: {len(dd.tokens)}")
    print(f"B matrix shape: {dd.B.shape}")
    print(f"B matrix is trainable: {dd.trainable_b}")

    # Training model
    print("\n" + "="*50)
    print("Starting Gradient Descent Training")
    print("="*50)
    dd.reg_train(seqs, t_list, max_iters=50, tol=1e-19, learning_rate=0.1, decay_rate = 0.999, batch_size=2048)    
    ...
    # Initialize new model for classification with trainable_b=False
    dd_cls = DualDescriptorPB(
        charset, 
        rank=rank, 
        vec_dim=vec_dim, 
        mode='nonlinear', 
        user_step=user_step,
        device='cuda' if torch.cuda.is_available() else 'cpu',
        trainable_b=False  # B matrix will not be trainable
    )
    ...
    dd_lbl = DualDescriptorPB(
        ...
        user_step=user_step,        
        device='cuda' if torch.cuda.is_available() else 'cpu',
        trainable_b=True  # B matrix will be trainable (default)
    )
    ...
    # Create a new model with trainable_b=True (default)
    dd_self = DualDescriptorPB(
        charset, 
        rank=rank, 
        vec_dim=vec_dim, 
        mode='nonlinear', 
