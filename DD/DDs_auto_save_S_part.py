...
class DualDescriptorScalar:
    ...
    def double_train(self, char_seqs, auto_mode='gap', part_mode='gap', 
        ...
        2. Converts character sequences to real-valued sequences using S()
        3. Then trains on these real-valued sequences using part_train
        ...
        # Stage 2: Convert to real-valued sequences and train
        print("\n=== Stage 2: Converting to real-valued sequences ===")
        num_seqs = []
        for seq in char_seqs:
            # Convert to S-sequence (cumulative position-weighted values)
            s_sequence = self.S(seq)
            num_seqs.append(s_sequence)
            print(f"Converted sequence (length {len(s_sequence)}): {s_sequence[:5]}...")
        
        print("\n=== Stage 2: Training on real-valued sequences ===")
        part_history = self.part_train(
            num_seqs,
            max_iters=max_iters2,
            learning_rate=learning_rate2,
            tol=tol2,
            continued=True,  # Always continue from stage 1 parameters
            auto_mode=part_mode
        )
...
##
    ...
    auto_history, part_history = dd.double_train(
        ...
        part_mode='reg',    # Second stage: predict next value
        max_iters1=100,
        max_iters2=100,
        learning_rate1=0.1,
        learning_rate2=0.1
    ...
    # Generate sequences using both components
    print("\nGenerated character sequence:")
    print(dd.generate(L=10, tau=0.5))

    print("\nGenerated real-valued sequence (gap mode):")
    print(dd.part_generate(L=5, mode='gap', tau=0.1))

    print("\nGenerated real-valued sequence (reg mode):")
    print(dd.part_generate(L=5, mode='reg', initial_value=0.0, tau=0.1))

    # Demonstrate the double generation capability
    def double_generate(dd, char_length=10, num_length=5, num_mode='gap', tau=0.1):
        """
        Generate a character sequence and then extend it with real values
        """
        # Generate character sequence
        char_seq = dd.generate(L=char_length, tau=tau)
        print(f"\nBase character sequence: {char_seq}")
        
        # Generate real-valued extension
        num_seq = dd.part_generate(L=num_length, mode=num_mode, tau=tau)
        print(f"Real-valued extension: {num_seq}")
        
        return char_seq, num_seq

    print("\n=== Double Generation Demo ===")
    char_part, num_part = double_generate(dd, char_length=8, num_length=4)
