...
class LinkedHierarchicalDD:
    ...
    def count_parameters(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            layer_params = m*L_basis + L_basis*m + m*n_in + L_in*L_out
            total += layer_params
            ...
            print(f"  Linker ({L_in}×{L_out}): {L_in*L_out:,} params")
...
if __name__ == "__main__":
    ...
    # Create linked hierarchical model
    print("\nCreating LinkedHierarchicalDD model...")
    lhdd = LinkedHierarchicalDD(
        input_dim=n,
        input_seq_len=L0,
    ...
    lhdd.count_parameters()
    
    # Train model
    print("\nTraining linked hierarchical model...")
    history = lhdd.grad_train(
        seqs, 
        t_list, 
        learning_rate=0.01,
        max_iters=500,
    ...
    preds = [lhdd.predict_t(seq) for seq in seqs]
    ...
    test_seq = [[random.uniform(-1,1) for _ in range(n)] for _ in range(L0)]
    test_t = [random.uniform(-1,1) for _ in range(m)]
    
    pred_t = lhdd.predict_t(test_seq)
