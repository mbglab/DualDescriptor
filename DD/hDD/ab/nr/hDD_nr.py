...
class HierDDab:
    ...
    """
    def __init__(self, input_dim=4, model_dims=[4], basis_dims=[50], use_residual_list=None):
        """
        ...
        Args:
            ...
            use_residual_list (list): List of residual connection types for each layer. 
                Options: 'separate', 'shared', or None
        """
        ...
        for i, out_dim in enumerate(model_dims):
            ...
            if use_residual == 'separate' and in_dim != out_dim:
                ...
                R = [[random.uniform(-0.1, 0.1) for _ in range(in_dim)]
                     for _ in range(out_dim)]
            ...
            self.layers.append({
                ...
                'R': R,            # Residual projection matrix (if any)
    ...
    def describe(self, vec_seq):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            R = layer['R']            # Residual projection matrix
            ...
            for k, vec in enumerate(current):
                ...
                # Compute scalar: B_j • normalized_vec
                ...
                if use_residual == 'separate':
                    if R is not None:
                        # Use separate projection matrix for residual
                        residual = self._mat_vec(R, vec)
                    else:
                    ...
                    # Use shared M matrix for residual
                    residual = transformed_vec  # M * vec
                    Nk = self._vec_add(Nk, residual)
                
                else:  # None or other
                    if len(vec) == len(Nk):  # Identity residual when dimensions match
    ...
    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, 
        ...
        if not continued:
            ...
            self.__init__(self.input_dim, self.model_dims, self.basis_dims, self.use_residual_list)
        ...
        for it in range(max_iters):
            ...
            for layer in self.layers:
                ...
                # Gradient for R matrix (if exists)
                grad_R = None
                if layer['R'] is not None:
            ...
            for seq, t in zip(seqs, t_list):
                ...
                for layer_idx, layer in enumerate(self.layers):
                    ...
                    for k, vec in enumerate(current):
                        ...
                        # Apply residual connection and save residual info
                        residual = None
                        residual_type = None
                        if use_residual == 'separate':
                            if R is not None:
                                ...
                                residual = vec  # Identity when dimensions match
                            Nk = self._vec_add(Nk, residual)
                            residual_type = 'separate'
                        elif use_residual == 'shared':
                            residual = transformed_vec  # M * vec
                            Nk = self._vec_add(Nk, residual)
                            residual_type = 'shared'
                        else:  # None or other
                            if len(vec) == len(Nk):  # Identity when dimensions match
                                residual = vec
                                Nk = self._vec_add(Nk, residual)
                ...
                for pos in range(len(seq)):
                    ...
                    for l_idx in range(self.num_layers-1, -1, -1):
                        ...
                        vec_mean = inter['mean']
                        vec_var = inter['var']
                        residual_type = inter['residual_type']
                        
                        # Handle residual connection gradient
                        if residual_type:
                            # Gradient flows through residual connection
                            if residual_type == 'separate' and R is not None:
                                # Compute gradient for R matrix
                                for d in range(out_dim):
                                    for e in range(in_dim):
                                        grad_R_list[l_idx][d][e] += dNk[d] * vec[e]
                                
                                # Propagate gradient to input
                                d_input_res = [0.0] * in_dim
                                for e in range(in_dim):
                                    for d in range(out_dim):
                                        d_input_res[e] += dNk[d] * R[d][e]
                            
                            elif residual_type == 'shared':
                                # Compute gradient for M matrix from residual path
                                for d in range(out_dim):
                                    for e in range(in_dim):
                                        grad_M_list[l_idx][d][e] += dNk[d] * vec[e]
                                
                                # Propagate gradient to input
                                d_input_res = [0.0] * in_dim
                                for e in range(in_dim):
                                    for d in range(out_dim):
                                        d_input_res[e] += dNk[d] * M[d][e]
                            
                            elif residual_type == 'identity':
                                # Direct gradient to input
                                d_input_res = dNk[:]  # Copy gradient
                        ...
                        # Backpropagation through normalization layer
                        ...
                        if l_idx > 0:
                            # Compute dL/dvec for previous layer from main path
                            d_prev_main = [0.0] * in_dim
                            for e in range(in_dim):
                                for d in range(len(d_transformed)):
                                    d_prev_main[e] += d_transformed[d] * M[d][e]
                            
                            # Combine gradients from main path and residual path
                            dNk = [d_prev_main[e] + d_input_res[e] for e in range(in_dim)]
                        else:
            ...
            for l_idx in range(self.num_layers):
                ...
                # Update R matrix if exists
                if R is not None and grad_R is not None:
                    for d in range(len(R)):
                        for e in range(len(R[d])):
                            R[d][e] -= current_lr * grad_R[d][e]
            ...
            if D >= D_prev - tol:
                ...
                for l_idx, layer in enumerate(self.layers):
                    ...
                    if layer['R'] is not None and current_params[l_idx]['R'] is not None:
                        layer['R'] = current_params[l_idx]['R']
    ...
    def show(self, what=None, first_num=5):
        ...
        if 'layers' in what or 'all' in what:
            for l_idx, layer in enumerate(self.layers):
                ...
                use_residual = layer['use_residual']
                has_R = layer['R'] is not None
                
                print(f"  Residual: {use_residual}, R matrix: {'Yes' if has_R else 'No'}")
                ...
                # Show R matrix sample if exists
                R = layer['R']
                if R is not None:
                    print(f"  R matrix ({len(R)}x{len(R[0])}):")
                    for i in range(min(first_num, len(R))):
                        row = R[i][:min(first_num, len(R[0]))]
                        print(f"    Row {i}: [{', '.join(f'{x:.4f}' for x in row)}" + 
                              (", ...]" if len(R[0]) > first_num else "]"))
    ...
    def count_parameters(self):
        ...
        for l_idx, layer in enumerate(self.layers):
            ...
            r_params = len(R) * len(R[0]) if R is not None else 0
            layer_params = m_params + a_params + r_params
            ...
            if R is not None:
                print(f"    R: {len(R)}×{len(R[0])} = {r_params} params")
...
if __name__ == "__main__":
    ...
    use_residual_list = ['shared', 'separate']  # Residual connection types for each layer
