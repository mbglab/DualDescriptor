...
class NumDualDescriptorAB:
    ...
    Model parameters:
      ...
      - Bbasis: bas_dim × model_dim matrix (internal basis, fixed)
    """
    ...
    def __init__(self, input_dim=4, output_dim=4, model_dim=4, basis_dim=50, rank=1, rank_op='avg', 
        ...
        # Bbasis[k][i] = cos(2π*(k+1)/(i+1)) - fixed and non-trainable
        self.Bbasis = []
        for k in range(self.L):
            row = []
            for i in range(self.m):
                # Formula: cos(2π*(k+1)/(i+1))
    ...
    def update_Bbasis(self, seqs, t_list):
        """Basis matrix is fixed - skip update"""
        # Bbasis is fixed and non-trainable
        return
    ...
    def train(self, seqs, t_list, max_iters=10, tol=1e-8, print_every=1):
        ...
        for it in range(max_iters):
            ...
            # Skip update_Bbasis since Bbasis is fixed
    ...
    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, 
        ...
        """
        if not continued:
            # Reinitialize parameters (except Bbasis which is fixed)
            self.M = [[random.uniform(-0.1, 0.1) for _ in range(self.n)] 
                      for _ in range(self.m)]
            self.Acoeff = [[random.uniform(-0.1,0.1) for _ in range(self.L)]
                           for _ in range(self.l)]
            # Bbasis remains fixed
        ...
        for it in range(max_iters):
            ...
            # Update parameters with gradients (skip Bbasis)
            for i in range(self.l):
    ...
    def dd_features(self, vec_seq, t=None):
        ...
        # 2. PWC coefficients (Acoeff ⊗ Bbasis) - skip Bbasis update
    ...
    def count_parameters(self):
        ...
        3. Bbasis matrix: L × m parameters (FIXED, not counted)
        Returns:
        ...
        print(f"- Bbasis (FIXED): 0 parameters")
