...
class NumDualDescriptorAB:
    ...
    Numeric Dual Descriptor for vector sequences with:
      ...
      - fixed basis matrix Bbasis ∈ R^{L×m} calculated as cos(2π*(k+1)/(i+1))
    ...
    def __init__(self, vec_dim=4, bas_dim=50, rank=1, rank_op='avg', rank_mode='drop', mode='linear', user_step=None):
        ...
        # Initialize fixed basis matrix Bbasis: L×m
        # Using cosine function: Bbasis[k][i] = cos(2π*(k+1)/(i+1))
        self.Bbasis = []
        for k in range(self.L):
            row = []
            for i in range(self.m):
                # Calculate fixed basis value using cosine formula
                value = math.cos(2 * math.pi * (k+1) / (i+1))
                row.append(value)
            self.Bbasis.append(row)
    ...
    def train(self, seqs, t_list, max_iters=10, tol=1e-8, print_every=1):
        ...
        for it in range(max_iters):
            ...
            self.update_M(seqs, t_list)  # Only update Acoeff and M
    ...
    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, 
        ...
        if not continued:
            ...
            self.Acoeff = [[random.uniform(-0.1,0.1) for _ in range(self.L)]
                           for _ in range(self.m)]
            # Bbasis remains fixed and is not reinitialized
        ...
        for it in range(max_iters):
            ...
            grad_M = [[0.0] * self.m for _ in range(self.m)]  # Removed grad_B
            total_loss = 0.0
            ...
            for seq, t in zip(seqs, t_list):
                ...
                for k, window_vec in enumerate(windows):
                    ...
                    # Compute scalar: B_j • x' (using fixed Bbasis)
                    ...
                    # Compute gradients (only for Acoeff and M)
                    for i in range(self.m):
                        # Gradient for Acoeff
            ...
            # Update parameters with gradients (only Acoeff and M)
            for i in range(self.m):
    ...
    def auto_train(self, seqs, auto_mode='reg', max_iters=1000, tol=1e-8,
        ...
        for it in range(max_iters):
            ...
            # Update parameters (only Acoeff and M)
            for i in range(self.m):
                for j in range(self.L):
                    self.Acoeff[i][j] -= current_lr * grad_A[i][j]
    ...
    def reconstruct(self, length):
        ...
        for k in range(length):
            ...
            for _ in range(100):
                ...
                # Compute scalar: B_j • transformed (using fixed Bbasis)
    ...
    def dd_features(self, vec_seq, t=None):
        ...
        # Bbasis is fixed, no need to backup
        self.update_Acoeff([vec_seq], [tg])  # Only update Acoeff
        p_flat = []
        for i in range(self.m):
            for j in range(self.m):
        ...
        feats['pwc'] = p_flat
        self.Acoeff = A_backup  # Restore Acoeff
    ...
    def count_parameters(self):
        ...
        2. M matrix: m × m parameters
        Note: Bbasis is fixed and not learnable        
        Returns:
