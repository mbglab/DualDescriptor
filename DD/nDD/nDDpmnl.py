# Copyright (C) Bin-Guang Ma (mbg@mail.hzau.edu.cn). All rights reserved.
# The Numeric Dual Descriptor class (Tensor form) with n-dim input and l-dim output
# Author: Bin-Guang Ma; Date: 2025-6-4

import math
import random
import pickle
from statistics import correlation, mean

class NumDualDescriptorPM:
    """
    Numeric Dual Descriptor for vector sequences with:
      - input_dim: dimension of input vectors (n)
      - output_dim: dimension of target vectors (l)
      - m: internal representation dimension
      - o: number of basis functions
      - P ∈ R^{l×m×o}: basis coefficient tensor
      - M ∈ R^{m×n}: linear transformation matrix
      - rank: window size for vector aggregation
      - indexed periods: period[i,j,g] = i*(m*o) + j*o + g + 2
      - basis function phi_{i,j,g}(k) = cos(2π * k / period[i,j,g])
    """
    def __init__(self, input_dim=4, output_dim=4, m=4, 
                 rank=1, rank_mode='drop', num_basis=5, 
                 mode='linear', user_step=None):
        """
        Initialize the Dual Descriptor for vector sequences.
        
        Args:
            input_dim (int): Dimensionality of input vectors (n)
            output_dim (int): Dimensionality of target vectors (l)
            m (int): Internal representation dimension
            rank (int): Window size for vector aggregation
            rank_mode (str): 'pad' or 'drop' for handling incomplete windows
            num_basis (int): Number of basis functions (o)
            mode (str): 'linear' or 'nonlinear' processing mode
            user_step (int): Custom step size for nonlinear mode
        """
        self.rank = rank
        self.rank_mode = rank_mode
        self.n = input_dim   # Input vector dimension
        self.l = output_dim  # Output vector dimension
        self.m = m           # Internal representation dimension
        self.o = num_basis   # Number of basis functions
        assert mode in ('linear','nonlinear')
        self.mode = mode
        self.step = user_step
        self.trained = False

        # Linear transformation matrix M ∈ R^{m×n}
        self.M = [[random.uniform(-0.5, 0.5) for _ in range(self.n)] 
                  for _ in range(self.m)]
        
        # Position-weight tensor P ∈ R^{l×m×o}
        self.P = [[[random.uniform(-0.1, 0.1) for _ in range(self.o)]
                   for _ in range(self.m)]
                  for _ in range(self.l)]

        # Precomputed indexed periods[i][j][g]
        self.periods = [[[ i*(self.m*self.o) + j*self.o + g + 2
                            for g in range(self.o)]
                           for j in range(self.m)]
                          for i in range(self.l)]

    def _invert(self, A):
        """Matrix inversion helper function"""
        n = len(A)
        M = [row[:] + [1.0 if i==j else 0.0 for j in range(n)]
             for i, row in enumerate(A)]
        for i in range(n):
            piv = M[i][i]
            if abs(piv) < 1e-12: continue
            M[i] = [x/piv for x in M[i]]
            for r in range(n):
                if r==i: continue
                fac = M[r][i]
                M[r] = [M[r][c] - fac*M[i][c] for c in range(2*n)]
        return [row[n:] for row in M]

    def _mat_vec(self, M, v):
        """Matrix-vector multiplication helper"""
        return [sum(M[i][j]*v[j] for j in range(len(v))) for i in range(len(M))]

    def _mat_mult(self, A, B):
        """Matrix multiplication helper"""
        n = len(A)
        p = len(B[0])
        m = len(B)
        C = [[0.0] * p for _ in range(n)]
        for i in range(n):
            for k in range(m):
                for j in range(p):
                    C[i][j] += A[i][k] * B[k][j]
        return C

    def extract_windows(self, seq):
        """
        Extract vector windows from a vector sequence.
        
        - 'linear': Slide window by 1 step, aggregating contiguous vectors
        - 'nonlinear': Slide window by custom step (or rank length if step not specified)
        
        Args:
            seq (list): List of n-dimensional vectors
            
        Returns:
            list: List of aggregated vectors (n-dimensional)
        """
        L = len(seq)
        windows = []
        
        # Linear mode: sliding window with step=1
        if self.mode == 'linear':
            for i in range(L - self.rank + 1):
                window = seq[i:i+self.rank]
                # Average vectors in window
                avg_vec = [sum(vec[d] for vec in window) / self.rank for d in range(self.n)]
                windows.append(avg_vec)
            return windows
        
        # Nonlinear mode: stepping with custom step size
        step = self.step or self.rank  # Use custom step if defined, else use rank length
        
        for i in range(0, L, step):
            window = seq[i:i+self.rank]
            if len(window) < self.rank:
                if self.rank_mode == 'pad':
                    # Pad window with zero vectors
                    pad_count = self.rank - len(window)
                    padded_window = window + [[0.0]*self.n for _ in range(pad_count)]
                    # Average vectors in padded window
                    avg_vec = [sum(vec[d] for vec in padded_window) / self.rank for d in range(self.n)]
                    windows.append(avg_vec)
                elif self.rank_mode == 'drop':
                    # Skip incomplete windows
                    continue
            else:
                # Average vectors in window
                avg_vec = [sum(vec[d] for vec in window) / self.rank for d in range(self.n)]
                windows.append(avg_vec)
                
        return windows

    def compute_Nk(self, k, vec):
        """Compute N(k) vector for a single position and vector (outputs l-dimensional vector)"""
        # Apply linear transformation: x = M * vec (m-dimensional)
        x = self._mat_vec(self.M, vec)
        # Initialize output vector (l-dimensional)
        Nk = [0.0] * self.l
        for i in range(self.l):
            for j in range(self.m):
                for g in range(self.o):
                    period = self.periods[i][j][g]
                    phi = math.cos(2 * math.pi * k / period)
                    Nk[i] += self.P[i][j][g] * x[j] * phi
        return Nk

    def describe(self, seq):
        """Compute N(k) vectors for each window in sequence (outputs l-dimensional vectors)"""
        windows = self.extract_windows(seq)
        N = []
        for k, vec in enumerate(windows):
            N.append(self.compute_Nk(k, vec))
        return N

    def S(self, seq):
        """
        Compute cumulative sum vectors S(l)=sum_{k=1}^{l} N(k) 
        for l = 1,...,L. Returns l-dimensional vectors.
        """
        Nk_list = self.describe(seq)        
        S = [0.0] * self.l
        S_list = []
        for l in range(len(Nk_list)):
            for i in range(self.l):
                S[i] += Nk_list[l][i]
            S_list.append(S.copy())
        return S_list 

    def deviation(self, seqs, t_list):
        """
        Compute mean squared deviation D across sequences:
        D = average over all positions of (N(k)-t_seq)^2
        t_seq are l-dimensional target vectors.
        """
        tot = 0.0
        cnt = 0
        for seq, t in zip(seqs, t_list):
            for Nk in self.describe(seq):
                for i in range(self.l):
                    d = Nk[i] - t[i]
                    tot += d * d
                cnt += 1
        return tot / cnt if cnt else 0.0

    def update_P(self, seqs, t_list):
        """
        Closed-form update of P tensor for vector sequences.
        """
        # Iterate over each output dimension i independently
        for i in range(self.l):
            dim = self.m * self.o  # Size of subsystem for this i
            U_i = [[0.0] * dim for _ in range(dim)]  # (m*o) x (m*o) matrix
            V_i = [0.0] * dim  # (m*o) vector
            
            # Accumulate data from all sequences and positions
            for seq, t in zip(seqs, t_list):
                windows = self.extract_windows(seq)
                for k, vec in enumerate(windows):
                    # Apply linear transformation: x = M * vec (m-dimensional)
                    x = self._mat_vec(self.M, vec)
                    # Build indices and basis values for current (i,k)
                    for j in range(self.m):
                        for g in range(self.o):
                            idx1 = j * self.o + g  # Linear index in subsystem
                            period = self.periods[i][j][g]
                            phi = math.cos(2 * math.pi * k / period)
                            a = x[j] * phi
                            
                            # Right-hand side vector component
                            V_i[idx1] += t[i] * a
                            
                            # Left-hand side matrix components
                            for j2 in range(self.m):
                                for h in range(self.o):
                                    idx2 = j2 * self.o + h
                                    period2 = self.periods[i][j2][h]  # Same i!
                                    phi2 = math.cos(2 * math.pi * k / period2)
                                    b = x[j2] * phi2
                                    U_i[idx1][idx2] += a * b
            
            # Solve subsystem for current i
            try:
                U_inv = self._invert(U_i)
                sol = [sum(U_inv[r][c] * V_i[c] for c in range(dim)) for r in range(dim)]
            except Exception as e:
                print(f"Warning: inversion failed for i={i}, using identity. Error: {str(e)}")
                sol = V_i[:]  # Fallback to identity solution
            
            # Update P tensor for current i
            for j in range(self.m):
                for g in range(self.o):
                    idx = j * self.o + g
                    self.P[i][j][g] = sol[idx]

    def update_M(self, seqs, t_list):
        """
        Closed-form update of transformation matrix M.
        """
        # Precompute basis products to avoid redundant calculations
        basis_cache = {}
        for i in range(self.l):
            for j in range(self.m):
                for g in range(self.o):
                    period = self.periods[i][j][g]
                    basis_cache[(i, j, g)] = period
        
        # Initialize gradient accumulation structures
        M_grad = [[0.0] * self.n for _ in range(self.m)]
        
        # Accumulate data from all sequences and positions
        total_positions = 0
        for seq, t in zip(seqs, t_list):
            windows = self.extract_windows(seq)
            total_positions += len(windows)
            for k, vec in enumerate(windows):
                # Precompute basis-parameter products
                psi = [[0.0] * self.m for _ in range(self.l)]  # ψ_{i,j} = Σ_g P[i][j][g] * φ_{ijg}(k)
                for i in range(self.l):
                    for j in range(self.m):
                        s = 0.0
                        for g in range(self.o):
                            period = basis_cache[(i, j, g)]
                            phi = math.cos(2 * math.pi * k / period)
                            s += self.P[i][j][g] * phi
                        psi[i][j] = s
                
                # Compute current transformed vector: x = M * vec (m-dimensional)
                x = self._mat_vec(self.M, vec)
                
                # Compute error terms (l-dimensional)
                error = [0.0] * self.l
                for i in range(self.l):
                    # Compute predicted Nk[i]
                    pred = 0.0
                    for j in range(self.m):
                        pred += psi[i][j] * x[j]
                    error[i] = pred - t[i]
                
                # Update gradient for M
                for d1 in range(self.m):  # Row in M
                    for d2 in range(self.n):  # Column in M
                        grad = 0.0
                        for i in range(self.l):
                            for j in range(self.m):
                                grad += error[i] * psi[i][j] * (1 if j == d1 else 0) * vec[d2]
                        M_grad[d1][d2] += grad
        
        # Normalize gradient by number of positions
        if total_positions > 0:
            for i in range(self.m):
                for j in range(self.n):
                    M_grad[i][j] /= total_positions
        
        # Update M using gradient (simple gradient descent)
        lr = 0.01  # Learning rate
        for i in range(self.m):
            for j in range(self.n):
                self.M[i][j] -= lr * M_grad[i][j]

    def train(self, seqs, t_list, max_iters=10, tol=1e-8, print_every=1):
        """Alternate closed-form updates for P and M"""        
        D_prev = float('inf')
        history = []
        for it in range(max_iters):
            self.update_P(seqs, t_list)
            self.update_M(seqs, t_list)
            D = self.deviation(seqs, t_list)
            history.append(D)
            if it % print_every == 0 or it == max_iters - 1:
                print(f"Iter {it:2d}: D = {D:.6e}")
            if D >= D_prev - tol:
                print("Converged.")
                break
            D_prev = D
            
        # Calculate statistics for reconstruction/generation
        total_window_count = sum(len(self.extract_windows(seq)) for seq in seqs)
        total_t = [0.0] * self.l
        for t_vec in t_list:
            for d in range(self.l):
                total_t[d] += t_vec[d]
        
        self.mean_window_count = total_window_count / len(seqs) if seqs else 0
        self.mean_t = [total_t[d] / len(t_list) for d in range(self.l)] if t_list else [0.0]*self.l
        self.trained = True        
        return history

    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, continued=False, decay_rate=1.0, print_every=10):
        """
        Train using gradient descent for vector sequences.
        """
        if not continued:
            # Initialize with small random values
            self.M = [[random.uniform(-0.5, 0.5) for _ in range(self.n)] 
                      for _ in range(self.m)]
            self.P = [[[random.uniform(-0.1, 0.1) for _ in range(self.o)]
                   for _ in range(self.m)]
                  for _ in range(self.l)]
            
        history = []
        D_prev = float('inf')
        total_positions = sum(len(self.extract_windows(seq)) for seq in seqs)
        
        for it in range(max_iters):
            # Initialize gradients
            grad_P = [[[0.0 for _ in range(self.o)] for _ in range(self.m)] for _ in range(self.l)]
            grad_M = [[0.0 for _ in range(self.n)] for _ in range(self.m)]
            
            # Accumulate gradients over all sequences and positions
            for seq, t_vec in zip(seqs, t_list):
                windows = self.extract_windows(seq)
                for pos, vec in enumerate(windows):
                    # Apply linear transformation: x = M * vec (m-dimensional)
                    x = self._mat_vec(self.M, vec)
                    Nk = [0.0] * self.l
                    
                    # Precompute basis functions and Nk
                    phi_vals = {}
                    for i in range(self.l):
                        for j in range(self.m):
                            for g in range(self.o):
                                period = self.periods[i][j][g]
                                phi = math.cos(2 * math.pi * pos / period)
                                phi_vals[(i, j, g)] = phi
                                Nk[i] += self.P[i][j][g] * x[j] * phi
                    
                    # Compute gradients
                    for i in range(self.l):
                        residual = 2 * (Nk[i] - t_vec[i]) / total_positions
                        for j in range(self.m):
                            for g in range(self.o):
                                phi = phi_vals[(i, j, g)]
                                # Gradient for P[i][j][g]
                                grad_P[i][j][g] += residual * x[j] * phi
                                
                                # Gradient for M (through chain rule)
                                grad_M_term = residual * self.P[i][j][g] * phi
                                for d in range(self.n):
                                    grad_M[j][d] += grad_M_term * vec[d]
            
            # Update P tensor
            for i in range(self.l):
                for j in range(self.m):
                    for g in range(self.o):
                        self.P[i][j][g] -= learning_rate * grad_P[i][j][g]
            
            # Update M matrix
            for i in range(self.m):
                for j in range(self.n):
                    self.M[i][j] -= learning_rate * grad_M[i][j]
            
            # Calculate current loss and print progress
            current_D = self.deviation(seqs, t_list)
            history.append(current_D)
            if it % print_every == 0 or it == max_iters - 1:
                print(f"GD Iter {it:4d}: D = {current_D:.6e}, LR = {learning_rate:.6f}")
            
            # Check convergence
            if D_prev - current_D < tol:
                print(f"Converged after {it+1} iterations.")
                break
            D_prev = current_D
            
            # Apply learning rate decay
            learning_rate *= decay_rate
        
        # Calculate statistics for reconstruction/generation
        total_t = [0.0] * self.l
        for t_vec in t_list:
            for d in range(self.l):
                total_t[d] += t_vec[d]
                
        self.mean_t = [total_t[d] / len(t_list) for d in range(self.l)] if t_list else [0.0]*self.l
        self.trained = True
        return history

    def auto_train(self, seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
                   continued=False, auto_mode='reg', decay_rate=1.0, print_every=10):
        """
        Self-training for vector sequences with two modes:
          - 'gap': Predicts current window's vector (requires n=l)
          - 'reg': Predicts next window's vector (requires n=l)
        """
        if auto_mode not in ('gap', 'reg'):
            raise ValueError("auto_mode must be either 'gap' or 'reg'")
            
        if self.n != self.l:
            raise ValueError("auto_train requires input_dim = output_dim")

        if not continued:
            # Initialize with small random values
            self.M = [[random.uniform(-0.5, 0.5) for _ in range(self.n)] 
                      for _ in range(self.m)]
            self.P = [[[random.uniform(-0.1, 0.1) for _ in range(self.o)]
                   for _ in range(self.m)]
                  for _ in range(self.l)]
        
        # Calculate total training samples
        total_samples = 0
        target_vectors = []
        for seq in seqs:
            windows = self.extract_windows(seq)
            if auto_mode == 'gap':
                total_samples += len(windows)  # All windows are samples
                target_vectors.extend(windows)
            else:  # 'reg' mode
                total_samples += max(0, len(windows) - 1)  # Windows except last
                if len(windows) > 1:
                    target_vectors.extend(windows[1:])
            
        if total_samples == 0:
            raise ValueError("No training samples found")
            
        history = []  # Store loss per iteration
        prev_loss = float('inf')
        
        for it in range(max_iters):
            # Initialize gradients
            grad_P = [[[0.0] * self.o for _ in range(self.m)] for _ in range(self.l)]
            grad_M = [[0.0] * self.n for _ in range(self.m)]
            total_loss = 0.0
            
            # Process all sequences
            for seq in seqs:
                windows = self.extract_windows(seq)
                if not windows:
                    continue
                    
                # Process windows based on mode
                for k in range(len(windows)):
                    # Skip last window in 'reg' mode
                    if auto_mode == 'reg' and k == len(windows) - 1:
                        continue
                        
                    current_vec = windows[k]
                    
                    # Compute N(k) for current window at position k
                    x = self._mat_vec(self.M, current_vec)
                    Nk = [0.0] * self.l
                    phi_vals = {}
                    for i in range(self.l):
                        for j in range(self.m):
                            for g in range(self.o):
                                period = self.periods[i][j][g]
                                phi = math.cos(2 * math.pi * k / period)
                                phi_vals[(i, j, g)] = phi
                                Nk[i] += self.P[i][j][g] * x[j] * phi
                    
                    # Determine target based on mode
                    if auto_mode == 'gap':
                        target = current_vec  # Current window vector (n-dimensional)
                    else:  # 'reg' mode
                        target = windows[k + 1]  # Next window vector (n-dimensional)
                    
                    # Compute loss and gradients
                    for i in range(self.l):
                        error = Nk[i] - target[i]
                        total_loss += error * error
                        
                        # Compute gradients (2 * error from derivative of squared loss)
                        grad_coeff = 2 * error / total_samples
                        
                        # Gradient for P[i][j][g]
                        for j in range(self.m):
                            for g in range(self.o):
                                phi = phi_vals[(i, j, g)]
                                grad_P[i][j][g] += grad_coeff * x[j] * phi
                                
                        # Gradient for M
                        for j in range(self.m):
                            for g in range(self.o):
                                phi = phi_vals[(i, j, g)]
                                grad_M_term = grad_coeff * self.P[i][j][g] * phi
                                for d in range(self.n):
                                    grad_M[j][d] += grad_M_term * current_vec[d]
            
            # Update parameters
            for i in range(self.l):
                for j in range(self.m):
                    for g in range(self.o):
                        self.P[i][j][g] -= learning_rate * grad_P[i][j][g]
            
            for i in range(self.m):
                for j in range(self.n):
                    self.M[i][j] -= learning_rate * grad_M[i][j]
            
            # Record and print progress
            avg_loss = total_loss / total_samples
            history.append(avg_loss)
            if it % print_every == 0 or it == max_iters - 1:
                mode_display = "Gap" if auto_mode == 'gap' else "Reg"
                print(f"AutoTrain({mode_display}) Iter {it:3d}: loss = {avg_loss:.6f}, LR = {learning_rate:.6f}")
            
            # Check convergence
            if prev_loss - avg_loss < tol:
                print(f"Converged after {it+1} iterations")
                break
            prev_loss = avg_loss
            
            # Apply learning rate decay
            learning_rate *= decay_rate
        
        # Compute and store statistics for reconstruction/generation
        total_t = [0.0] * self.l
        for vec in target_vectors:
            for d in range(self.l):
                total_t[d] += vec[d]
        
        self.mean_t = [t_val / len(target_vectors) for t_val in total_t] if target_vectors else [0.0]*self.l
        self.trained = True
        
        return history

    def predict_t(self, seq):
        """
        Predict target vector for a sequence.
        Returns the average of all N(k) vectors in the sequence (l-dimensional).
        """
        N_list = self.describe(seq)
        if not N_list:
            return [0.0] * self.l
            
        # Average all N(k) vectors component-wise
        sum_t = [0.0] * self.l
        for Nk in N_list:
            for d in range(self.l):
                sum_t[d] += Nk[d]
        N = len(N_list)
        return [ti / N for ti in sum_t]   

    def generate(self, num_windows, tau=0.0):
        """
        Generate sequence of l-dimensional vectors.
        Uses mean target vector with temperature-controlled randomness.
        """
        assert self.trained, "Model must be trained first"
        if tau < 0:
            raise ValueError("Temperature must be non-negative")            
        
        generated_vectors = []
        for _ in range(num_windows):
            vec = []
            for i in range(self.l):
                if tau == 0:
                    # Use mean value without randomness
                    vec.append(self.mean_t[i])
                else:
                    # Add Gaussian noise scaled by temperature
                    vec.append(random.gauss(self.mean_t[i], tau))
            generated_vectors.append(vec)
        return generated_vectors

    def dd_features(self, seq, t=None):
        """
        Extract feature vector for a vector sequence.
        (Placeholder implementation)
        """
        # In a real implementation, this would return features
        # derived from the model and sequence
        return [0.0] * (self.l * self.m * self.o + self.m * self.n)

    def show(self):
        """Display model configuration."""
        print("NumDualDescriptorPM Status:")
        print(f"  Input dimension n = {self.n}")
        print(f"  Output dimension l = {self.l}")
        print(f"  Internal dimension m = {self.m}")
        print(f"  Basis functions o = {self.o}")
        print(f"  Trained: {self.trained}")

    def count_parameters(self):
        """Count learnable parameters (P tensor and M matrix)."""
        total_params = self.l * self.m * self.o + self.m * self.n
        print(f"Parameter Count:")
        print(f"  P tensor: {self.l}×{self.m}×{self.o} = {self.l*self.m*self.o} parameters")
        print(f"  M matrix: {self.m}×{self.n} = {self.m*self.n} parameters")
        print(f"Total parameters: {total_params}")
        return total_params

    def save(self, filename):
        """Save model state to file"""
        with open(filename, 'wb') as f:
            pickle.dump(self.__dict__, f)
        print(f"Model saved to {filename}")

    @classmethod
    def load(cls, filename):
        """Load model state from file"""
        with open(filename, 'rb') as f:
            state = pickle.load(f)
        
        # Create new instance without calling __init__
        obj = cls.__new__(cls)
        # Restore saved state
        obj.__dict__.update(state)
        print(f"Model loaded from {filename}")
        return obj

# === Example Usage ===
if __name__=="__main__":
    # Configuration with different input/output dimensions
    input_dim = 3    # Input vector dimension (n)
    output_dim = 2   # Target vector dimension (l)
    m = 5           # Internal representation dimension
    num_basis = 5    # Number of basis functions
    num_seqs = 10    # Number of training sequences
    min_len, max_len = 100, 200  # Sequence length range

    random.seed(3)

    # Generate synthetic training data
    seqs = []    # List of sequences (n-dimensional vectors)
    t_list = []  # List of target vectors (l-dimensional)
    for _ in range(num_seqs):
        L = random.randint(min_len, max_len)
        seq = [[random.uniform(-1,1) for _ in range(input_dim)] for __ in range(L)]
        seqs.append(seq)
        t_list.append([random.uniform(-1,1) for _ in range(output_dim)])

    # Create model
    print("Creating model...")
    dd = NumDualDescriptorPM(
        input_dim=input_dim,
        output_dim=output_dim,
        m=m,
        rank=3,
        num_basis=num_basis,
        mode='linear'
    )
    
    # Train the model with closed-form updates
    print("\nTraining with closed-form updates...")
    #seqs.reverse(); t_list.reverse()
    history = dd.train(seqs, t_list, max_iters=10, print_every=1)
    
    # Show model status
    dd.show()

    # Count learnable parameters
    dd.count_parameters()
    
    # Predict targets
    print("\nPredictions for first 3 sequences:")
    predictions = []
    for i, seq in enumerate(seqs[:3]):
        t_pred = dd.predict_t(seq)
        predictions.append(t_pred)
        print(f"Sequence {i+1}:")
        print(f"  Target:    {[round(x,4) for x in t_list[i]]}")
        print(f"  Predicted: {[round(x,4) for x in t_pred]}")
    
    # Calculate prediction correlations
    all_preds = [dd.predict_t(seq) for seq in seqs]
    correlations = []
    for i in range(output_dim):
        actual = [t[i] for t in t_list]
        predicted = [p[i] for p in all_preds]
        corr = correlation(actual, predicted) if len(actual) > 1 else 0.0
        correlations.append(corr)
        print(f"Dimension {i} correlation: {corr:.4f}")
    print(f"Average correlation: {mean(correlations):.4f}")
    
    # Train with Gradient Descent
    print("\nTraining with Gradient Descent...")
    dd_grad = NumDualDescriptorPM(
        input_dim=input_dim,
        output_dim=output_dim,
        m=m,
        rank=3,
        num_basis=num_basis
    )
    grad_history = dd_grad.grad_train(
        seqs, 
        t_list, 
        learning_rate=0.2,
        max_iters=200,
        decay_rate=0.999,
        print_every=20
    )

    # Calculate prediction correlations for GD model
    gd_preds = [dd_grad.predict_t(seq) for seq in seqs]
    gd_correlations = []
    for i in range(output_dim):
        actual = [t[i] for t in t_list]
        predicted = [p[i] for p in gd_preds]
        corr = correlation(actual, predicted) if len(actual) > 1 else 0.0
        gd_correlations.append(corr)
        print(f"GD Model Dim {i} correlation: {corr:.4f}")
    print(f"GD Model Average correlation: {mean(gd_correlations):.4f}")
    
    # Generate new sequence
    print("\nGenerating new sequence of output vectors...")
    generated_seq = dd.generate(num_windows=5, tau=0.1)
    print("Generated l-dimensional vectors:")
    for i, vec in enumerate(generated_seq):
        print(f"Vec {i+1}: {[round(x,4) for x in vec]}")

    # Save and load model
    print("\nSaving and loading model...")
    dd.save("nddpm_model.pkl")
    loaded_model = NumDualDescriptorPM.load("nddpm_model.pkl")
    
    # Verify loaded model
    test_pred = loaded_model.predict_t(seqs[0])
    print(f"Prediction from loaded model: {[round(x,4) for x in test_pred]}")
