...
class NumDualDescriptorPM:
    ...
    """
    def __init__(self, in_dim=2, out_dim=2, rank=1, rank_op='avg', rank_mode='drop', mode='linear', user_step=None):        
    ...
    def update_P(self, seqs, t_list):
        ...
        for i in range(self.m):
            dim = self.m  # Size of subsystem for this i (m×m matrix reduced to m)
            U_i = [[0.0] * dim for _ in range(dim)]  # m×m matrix
    ...
    def show(self, what=None, first_num=5):
        ...
        for attr in attrs:
            if attr == 'config':
                ...
                print("\n[Weight Matrix (P)]")
                print(f"Shape: {len(self.P)}x{len(self.P[0])} (m x m)")
                print("Sample values:")
                for i in range(min(first_num, len(self.P))):
                    vals = self.P[i][:min(first_num, len(self.P[i]))]
                    print(f"  Row {i}: [{', '.join(f'{v:.6f}' for v in vals)}" + 
                          (f", ...]" if len(self.P[i]) > first_num else "]"))
            
            elif attr == 'periods':
                # Display basis function periods
                print("\n[Basis Periods]")
                print(f"Shape: {len(self.periods)}x{len(self.periods[0])} (m x m)")
                print("Sample values:")
                for i in range(min(first_num, len(self.periods))):
                    vals = self.periods[i][:min(first_num, len(self.periods[i]))]
                    print(f"  periods[{i}][:]: {vals}" + 
                          (f", ..." if len(self.periods[i]) > first_num else ""))
    ...
    def count_parameters(self):
        ...
        total_params = self.m * self.m + self.m * self.n
        print(f"Parameter Count:")
        print(f"  P matrix: {self.m}×{self.m} = {self.m*self.m} parameters")
