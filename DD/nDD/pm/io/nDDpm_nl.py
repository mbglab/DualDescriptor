...
class NumDualDescriptorPM:
    ...
    Numeric Dual Descriptor for vector sequences with:
      ...
      - P ∈ R^{l×m}: basis coefficient matrix
    ...
    def __init__(self, input_dim=4, output_dim=4, model_dim=4, 
                 rank=1, rank_op='avg', rank_mode='drop', 
        ...
        # Position-weight matrix P ∈ R^{l×m}
        self.P = [[random.uniform(-0.1, 0.1) for _ in range(self.m)]
                  for _ in range(self.l)]

        # Precomputed indexed periods[i][j]
    ...
    def update_P(self, seqs, t_list):
        """
        Closed-form update of P matrix for vector sequences.
        """
    ...
    def update_M(self, seqs, t_list):
        ...
        for seq, t in zip(seqs, t_list):
            ...
            for k, vec in enumerate(windows):
                ...
                psi = [[0.0] * self.m for _ in range(self.l)]  # ψ_{i,j} = P[i][j] * φ_{ij}(k)
                for i in range(self.l):
                    for j in range(self.m):
    ...
    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, continued=False, decay_rate=1.0, print_every=10):
        ...
        for it in range(max_iters):
            ...
            grad_P = [[0.0 for _ in range(self.m)] for _ in range(self.l)]
    ...
    def dd_features(self, seq, t=None):
        ...
        return [0.0] * (self.l * self.m + self.m * self.n)
    ...
    def count_parameters(self):
        ...
        total_params = self.l * self.m + self.m * self.n
