...
class NumDualDescriptorPM:
        ...
        for i in range(self.m):
            ...
            for seq, t in zip(seqs, t_list):
                ...
                for k, vec in enumerate(windows):
                    ...
                    for j in range(self.m):
                        idx = j  # Linear index in subsystem
                        period = self.periods[i][j]
                        phi = math.cos(2 * math.pi * k / period)
                        a = x[j] * phi
    ...
    def update_M(self, seqs, t_list):
        ...
        dim = self.m * self.m  # Number of elements in M
        ...
        for seq, t in zip(seqs, t_list):
            ...
            for k, vec in enumerate(windows):
                ...
                for d1 in range(self.m):  # Row index in M
                    for d2 in range(self.m):  # Column index in M
                        idx1 = d1 * self.m + d2  # Linear index in flattened M
                        ...
                        for e1 in range(self.m):
                            for e2 in range(self.m):
                                idx2 = e1 * self.m + e2  # Linear index in flattened M
                                a_contrib = 0.0
                                for i in range(self.m):
    ...
    def show(self, what=None, first_num=5):
        ...
        for attr in attrs:
            if attr == 'config':
                ...
                print("\n[Weight Matrix (P)]")
                print(f"Shape: {len(self.P)}x{len(self.P[0])}")
                print("Sample values:")
                ...
                print("\n[Basis Periods]")
                print(f"Shape: {len(self.periods)}x{len(self.periods[0])}")
                print("Sample values:")
...
if __name__=="__main__":
    ...
    dd_grad = NumDualDescriptorPM(vec_dim=vec_dim, rank=rank, rank_op='avg')
    ...
    dd_auto = NumDualDescriptorPM(vec_dim=vec_dim, rank=rank)
