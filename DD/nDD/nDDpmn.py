...
# The Numeric Dual Descriptor class (Tensor form) for n-dimensional vectors
# Modified to support n-dimensional input and output with m-dimensional internal representation
...
class NumDualDescriptorPM:
    ...
    - num_basis: number of basis functions (o)
    Model:
        For each vector x_k in sequence:
            z_k = M * x_k (m-dimensional internal representation)
            N(k)[i] = Σ_j Σ_g P[i][j][g] * z_k[j] * cos(2πk / period[i][j][g])
            S(l) = cumulative sum of N(k) from k=1 to l
    """
    def __init__(self, input_dim=2, model_dim=2, rank=1, rank_mode='drop', num_basis=5, mode='linear', user_step=None):
        """
        ...
        # Position-weight tensor P[i][j][g] ∈ R^{n×m×o}
    ...
    def extract_windows(self, seq):
        """
        Extract vector windows from a sequence of n-dimensional vectors.
        ...
        vec_dim = len(seq[0])  # Dimension of vectors in sequence
        L = len(seq)
        windows = []
        ...
        if self.mode == 'linear':
            ...
            for i in range(L - self.rank + 1):
                ...
                # Average vectors in window (n-dimensional)
                avg_vec = [sum(vec[d] for vec in window) / self.rank for d in range(vec_dim)]
        ...
        for i in range(0, L, step):
            ...
            if len(window) < self.rank:
                if self.rank_mode == 'pad':
                    # Pad window with zero vectors (n-dimensional)
                    pad_count = self.rank - len(window)
                    padded_window = window + [[0.0]*vec_dim for _ in range(pad_count)]
                    # Average vectors in padded window
                    avg_vec = [sum(vec[d] for vec in padded_window) / self.rank for d in range(vec_dim)]
                    windows.append(avg_vec)
    ...
    def compute_Nk(self, k, vec):
        ...
        for i in range(self.n):      # Output dimension
            for j in range(self.m):  # Internal dimension
                for g in range(self.o):  # Basis index
                    period = self.periods[i][j][g]
                    phi = math.cos(2 * math.pi * k / period)
                    Nk[i] += self.P[i][j][g] * z[j] * phi
        return Nk
    ...
    def update_P(self, seqs, t_list):
        """Closed-form update of P tensor for n-dimensional vectors"""
    ...
    def update_M(self, seqs, t_list):
        ...
        for seq, t in zip(seqs, t_list):
            ...
            for pos, vec in enumerate(windows):
                ...
                psi = [[0.0] * self.m for _ in range(self.n)]  # ψ_{i,j} = Σ_g P[i][j][g] * φ_{ijg}(pos)
                for i in range(self.n):
                    for j in range(self.m):
                        s = 0.0
                        for g in range(self.o):
                ...
                for row in range(self.m):    # M row index
                    for col in range(self.n):  # M column index
                        grad = 0.0
                        for i in range(self.n):
                            grad += error[i] * psi[i][row] * vec[col]
                        M_grad[row][col] += grad
    ...
    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        for it in range(max_iters):
            ...
            grad_P = [[[0.0 for _ in range(self.o)] for _ in range(self.m)] for _ in range(self.n)]
    ...
    def auto_train(self, seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
        ...
        for it in range(max_iters):
            # Initialize gradients
            grad_P = [[[0.0] * self.o for _ in range(self.m)] for _ in range(self.n)]
                ...
                for k in range(len(windows)):
                    ...
                    for i in range(self.n):
                        ...
                        for j in range(self.m):
                            for g in range(self.o):
                                phi = phi_vals[(i, j, g)]
                                grad_P[i][j][g] += grad_coeff * z[j] * phi
    ...
    def dd_features(self, seq):
        ...
        features = []
        # Flatten P tensor (n×m×o)
        for i in range(self.n):
            for j in range(self.m):
                features.extend(self.P[i][j])
    ...
    def count_parameters(self):
        ...
        total_params = self.n * self.m * self.o + self.m * self.n
        print(f"Parameter Count:")
        print(f"  P tensor: {self.n}×{self.m}×{self.o} = {self.n*self.m*self.o}")
