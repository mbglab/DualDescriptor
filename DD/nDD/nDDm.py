...
class NumDualDescriptor:
    ...
    def __init__(self, model_dim=4, input_dim=4, rank=1, rank_mode='drop', rank_op='avg', mode='linear', user_step=None):
        ...
        Args:
            vec_dim (int): Internal dimensionality of vectors (m)
    ...
    def update_M(self, seqs, t_list):
        ...
        # Initialize accumulation matrices
        sum_vvT = [[0.0] * n for _ in range(n)]  # n×n (changed from m×m)
        sum_tvT = [[0.0] * n for _ in range(m)]   # m×n (changed from m×m)
        ...
        for it in range(max_iters):
            ...
            for seq in seqs:
                ...
                for k in indices:
                    ...
                    # Compute error - NOTE: target is n-dim but output is m-dim
                    # This requires autoencoder mode to use m-dim targets
                    error = [N_i - target_vec_i for N_i, target_vec_i in zip(N, target_vec[:self.m])]
                    total_loss += sum(e*e for e in error)
                    total_instances += 1
    ...
    def generate(self, length, init_vec=None, tau=0.0):
        ...
        Returns:
            list: Generated sequence of vectors (m-dimensional)
        """
        ...
        for _ in range(length - 1):
            ...
            next_pred = self._mat_vec(self.Acoeff, z)  # m-dim output
...
if __name__ == "__main__":
    ...
    model_dim = 10  # Internal model dimension (m)
    n_seqs = 10    # Number of training sequences

    # Generate sequences with n-dimensional vectors and m-dimensional targets
    seqs, t_list = [], []
    for _ in range(n_seqs):
        ...
        seqs.append(seq)
        # Create m-dimensional target vector
