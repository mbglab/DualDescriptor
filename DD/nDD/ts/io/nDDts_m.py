...
class NumDualDescriptorTS:
    ...
    """
    def __init__(self, in_dim=2, out_dim=2, num_basis=5, rank=1, rank_op='avg', rank_mode='drop', mode='linear', user_step=None):        
    ...
    def update_M(self, seqs, t_list):
        ...
        dim = self.m * self.n  # Number of elements in M (m x n)
        ...
        for seq, t in zip(seqs, t_list):
            ...
            for k, vec in enumerate(windows):
                ...
                for d1 in range(self.m):  # Row index in M (output dimension)
                    for d2 in range(self.n):  # Column index in M (input dimension)
                        ...
                        for e1 in range(self.m):  # Row index in M
                            for e2 in range(self.n):  # Column index in M
...
if __name__=="__main__":
    ...
    dd = NumDualDescriptorTS(in_dim=in_dim, out_dim=out_dim, rank=rank, 
    ...
    gen_vecs = dd.generate(num_windows=3, tau=0.1)
    ...
    dd_grad = NumDualDescriptorTS(in_dim=in_dim, out_dim=out_dim, 
    ...
    dd_auto = NumDualDescriptorTS(in_dim=in_dim, out_dim=out_dim, 
                                 rank=rank, num_basis=num_basis)
