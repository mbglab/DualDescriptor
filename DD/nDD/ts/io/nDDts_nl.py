...
class NumDualDescriptorTS:
    ...
    """
    def __init__(self, input_dim=4, output_dim=4, model_dim=4, 
                 rank=1, rank_op='avg', rank_mode='drop', num_basis=5, 
    ...
    def update_M(self, seqs, t_list):
        ...
        For each output dimension i, position k, and input vector vec, we have:
            Nk[i] = sum_{j,g} [ P[i][j][g] * (M * vec)[j] * phi_{ijg}(k) ]
        ...
        dim = self.m * self.n  # Number of elements in M (m×n matrix)
        ...
        for i in range(self.l):
            for j in range(self.m):
                for g in range(self.o):
                    ...
                    basis_cache[(i, j, g)] = phi_vals

        # Accumulate data from all sequences and positions
        for seq, t in zip(seqs, t_list):
            windows = self.extract_windows(seq)
            for k, vec in enumerate(windows):
                # Precompute the product of P and basis functions for all i,j
                psi = [[0.0] * self.m for _ in range(self.l)]  # ψ_{i,j} = Σ_g P[i][j][g] * φ_{ijg}(k)
                for i in range(self.l):
                    for j in range(self.m):
                        s = 0.0
                        for g in range(self.o):
                            phi_val = basis_cache[(i, j, g)][k]
                            s += self.P[i][j][g] * phi_val
                        psi[i][j] = s
                    ...
                    for d2 in range(self.n):  # Column index in M
                        ...
                        for e1 in range(self.m):
                            for e2 in range(self.n):
                                ...
                                a_contrib = 0.0
                                for i in range(self.l):
                                    a_contrib += psi[i][d1] * psi[i][e1] * vec[d2] * vec[e2]
    ...
    def generate(self, num_windows, tau=0.0):
        ...
        for k in range(num_windows):
            ...
            candidates = []
            for _ in range(100):
                candidate = [random.gauss(self.mean_t[d], 0.1) for d in range(self.l)]
...
if __name__=="__main__":
    ...
    model_dim = 5           # Internal representation dimension
