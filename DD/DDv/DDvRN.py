...
# The Dual Descriptor Vector class (Random AB matrix form) implemented with pure Python
...
class DualDescriptorRN:
    ...
    def grad_train(self, seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, continued=False, decay_rate=1.0, print_every=10):
        """
        Train the DualDescriptorRN model using gradient descent optimization.
        
        This method updates all three learnable components: 
          - Coefficient matrix Acoeff
          - Basis matrix Bbasis 
          - Token embeddings M
        
        Key features:
        1. Uses position-level normalization for stable gradients
        2. Handles variable-length sequences
        3. Includes learning rate decay
        4. Supports continued training from current parameters
        
        Args:
            seqs: List of input sequences
            t_list: List of target vectors (m-dimensional)
            max_iters: Maximum training iterations
            tol: Convergence tolerance
            learning_rate: Initial learning rate
            continued: Continue training from current state
            decay_rate: Learning rate decay factor
            print_every: Progress printing frequency
        ...
        # Compute total tokens across all sequences
        total_positions = sum(len(self.extract_tokens(seq)) for seq in seqs)
        if total_positions == 0:
            raise ValueError("No tokens found in sequences")
        
        history = []  # Loss history
        D_prev = float('inf')  # Previous loss
        
        for it in range(max_iters):
            # Initialize gradients for all parameters
            grad_A = [[0.0] * self.L for _ in range(self.m)]  # Acoeff gradient
            grad_B = [[0.0] * self.m for _ in range(self.L)]  # Bbasis gradient
            grad_M = {tok: [0.0] * self.m for tok in self.tokens}  # M gradient
            ...
            for seq, t in zip(seqs, t_list):
                toks = self.extract_tokens(seq)
                if not toks:  # Skip empty token lists
                    continue
                    
                for k, tok in enumerate(toks):
                    j = k % self.L  # Basis index
                    x = self.M[tok]  # Token embedding
                    
                    # Compute scalar projection: B_j • x
                    scalar = sum(self.Bbasis[j][i] * x[i] for i in range(self.m))
                    
                    # Compute position descriptor: Nk = scalar * A_j
                    A_j = [self.Acoeff[i][j] for i in range(self.m)]  # j-th column
                    Nk = [A_j[i] * scalar for i in range(self.m)]
                    
                    # Compute position loss (MSE per position)
                    pos_loss = 0.0
                    for i in range(self.m):
                        diff = Nk[i] - t[i]
                        pos_loss += diff * diff
                    total_loss += pos_loss / self.m  # Normalize by vector dimension
                    
                    # Compute gradient components
                    for i in range(self.m):
                        # Error derivative (2 * error / m)
                        error_i = 2 * (Nk[i] - t[i]) / self.m
                        
                        # Gradient for Acoeff[i][j]
                        grad_A[i][j] += error_i * scalar
                        
                        # Gradient for Bbasis[j] and M[tok]
                        factor = error_i * A_j[i]
                        for l in range(self.m):
                            # ∂L/∂B_jl
                            grad_B[j][l] += factor * x[l]
                            # ∂L/∂x_l
                            grad_M[tok][l] += factor * self.Bbasis[j][l]
            ...
            for j in range(self.L):
                for l in range(self.m):
                    self.Bbasis[j][l] -= learning_rate * grad_B[j][l]
            
            for tok in self.tokens:
                for l in range(self.m):
                    self.M[tok][l] -= learning_rate * grad_M[tok][l]
    ...
    def auto_train(self, seqs, auto_mode='gap', max_iters=1000, tol=1e-8, 
        ...
        # Prepare training samples: (sequence, position, target_token)
        samples = []
        for seq in seqs:
            toks = self.extract_tokens(seq)
            if not toks:
        ...
        for it in range(max_iters):
            ...
            for seq, k, target_tok in samples:
                ...
                if k >= len(context_toks):
                    continue
                input_tok = context_toks[k]  # Input token for prediction
                j = k % self.L  # Basis index for this position
                
                # Retrieve embeddings
                input_vec = self.M[input_tok]
                target_vec = self.M[target_tok]
                
                # Compute scalar projection: B_j • input_vec
                scalar = sum(self.Bbasis[j][i] * input_vec[i] for i in range(self.m))
                
                # Compute position descriptor: Nk = scalar * A_j
                A_j = [self.Acoeff[i][j] for i in range(self.m)]  # j-th column of A
                Nk = [A_j[i] * scalar for i in range(self.m)]
                
                # Compute loss and gradients
                loss_val = 0.0
                dNk = [0.0] * self.m  # Gradient of loss w.r.t Nk
                for i in range(self.m):
                    error = Nk[i] - target_vec[i]
                    loss_val += error * error
                    dNk[i] = 2 * error / self.m  # Normalized gradient
                total_loss += loss_val / self.m  # Average over dimensions
                ...
                for i in range(self.m):
                    ...
                    # Gradient for Bbasis and input embedding
                    factor = dNk[i] * A_j[i]
                    for l in range(self.m):
                        # ∂L/∂B_jl
                        grad_B[j][l] += factor * input_vec[l]
                        # ∂L/∂input_vec[l]
                        grad_M[input_tok][l] += factor * self.Bbasis[j][l]
                
                # Gradient for target token embedding
                for i in range(self.m):
                    grad_M[target_tok][i] -= dNk[i]  # Negative gradient
            ...
            for j in range(self.L):
                for l in range(self.m):
                    self.Bbasis[j][l] -= current_lr * grad_B[j][l]
            
            for tok in self.tokens:
                for i in range(self.m):
                    self.M[tok][i] -= current_lr * grad_M[tok][i]
    ...
    def reconstruct(self):
        ...
        for k in range(n_tokens):
            ...
            for tok in self.tokens:
                # Compute scalar: Bbasis[j] • M[tok]
                scalar = sum(self.Bbasis[j][i] * self.M[tok][i] 
                            for i in range(self.m))
    ...
    def generate(self, L, tau=0.0):
        ...
        # Calculate number of tokens needed (each token is of length=rank)
    ...
    def dd_features(self, seq, t=None):
        ...
        # 2. PWC coefficients (pwc): Flattened Acoeff ⊗ Bbasis
        A_backup = self.Acoeff.copy()
        B_backup = self.Bbasis.copy()
        self.update_Acoeff([seq], [tg])
        self.update_Bbasis([seq], [tg])
        p_flat = []
        # Construct 3D tensor P[i][j][g] = Acoeff[i][g] * Bbasis[g][j]
        for i in range(self.m):
        ...
        # 3. Token embeddings (cwf): Flattened M values
        M_backup = self.M.copy()        
        self.update_M([seq], [tg])  
        M_flat = []
        for tok in sorted(self.tokens):  # Consistent ordering
            M_flat.extend(self.M[tok])
        feats['cwf'] = M_flat
        self.M = M_backup
    ...
    def part_train(self, vec_seqs, max_iters=100, tol=1e-6, learning_rate=0.01,
        ...
        """
        Trains the interaction model using gradient descent on vector sequences.
        ...
        # Initialize parameters if not continuing
        if not continued or not hasattr(self, 'AcoeffI'):
            self.AcoeffI = [[random.uniform(-0.1, 0.1) for _ in range(L)] 
                            for _ in range(m)]
            self.BbasisI = [[random.uniform(-0.1, 0.1) for _ in range(m)]
                            for _ in range(L)]
        ...
        for it in range(max_iters):
            # Initialize gradients
            grad_A = [[0.0] * L for _ in range(m)]  # AcoeffI gradient
            grad_B = [[0.0] * m for _ in range(L)]   # BbasisI gradient
            total_loss = 0.0
            ...
            for seq in vec_seqs:
                ...
                for k in range(len(seq)):
                    ...
                    # Get basis vector and current vector
                    basis_idx = k % L
                    basis_vec = self.Bbasis[basis_idx]  # Original basis
                    current_vec = seq[k]
                    ...
                    for g in range(L):
                        for j in range(m):
                            proj[g] += self.BbasisI[g][j] * current_vec[j]
                    
                    # Compute Nk = AcoeffI • proj
                    Nk = [0.0] * m
                    for i in range(m):
                        for g in range(L):
                            Nk[i] += self.AcoeffI[i][g] * proj[g]
                    ...
                    for i in range(m):
                        ...
                        # Gradient for AcoeffI
                        for g in range(L):
                            grad_A[i][g] += grad_coeff * proj[g]
                        
                        # Gradient for BbasisI
                        for g in range(L):
                            grad_val = grad_coeff * self.AcoeffI[i][g]
                            for j in range(m):
                                grad_B[g][j] += grad_val * current_vec[j]
            
            # Apply gradient updates
            for i in range(m):
                for g in range(L):
                    self.AcoeffI[i][g] -= current_lr * grad_A[i][g]
            
            for g in range(L):
                for j in range(m):
                    self.BbasisI[g][j] -= current_lr * grad_B[g][j]
        ...
        self.mean_vector = [s / total_vecs for s in sum_vec]
        return history

    def part_generate(self, L, tau=0.0, mode='reg'):
        """
        Generates a sequence of vectors using the trained interaction model.
        ...
        if mode == 'gap':
            ...
            for k in range(L):
                ...
                if tau > 0:
                    Nk = [val + random.gauss(0, tau) for val in Nk]
            ...
            for k in range(L):
                ...
                for i in range(m):
                    for g in range(self.L):
                        next_vec[i] += self.AcoeffI[i][g] * proj[g]
    ...
    def double_generate(self, L, tau=0.0, mode='reg'):
        ...
        def compute_Nk(k, token):
            j = k % self.L
            scalar = sum(self.Bbasis[j][i] * self.M[token][i] for i in range(self.m))
...
if __name__ == "__main__":
    ...
    dd_grad = DualDescriptorRN(charset, rank=1, vec_dim=2, bas_dim=150, mode='linear', user_step=None)
    ...
    # Load model
    dd_loaded = DualDescriptorRN.load("auto_trained_model.pkl")
    ...
    # Create new model
    dd_part = DualDescriptorRN(charset="", rank=1, vec_dim=2)
