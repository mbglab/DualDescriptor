...
class DualDescriptorTSR:
    ...
    """
    def __init__(self, charset, rank=1, vec_dim=2, num_basis=5, mode='linear', user_step=None):
        ...
        # build r-per/k-mer vocabulary of fixed length = rank
        toks = [''.join(p) for p in itertools.product(self.charset, repeat=rank)]
    ...
    def extract_tokens(self, seq):
        ...
        'nonlinear': slide by rank or user step 
        """
        L = len(seq)
        if self.mode=='linear':
    ...
    def update_P(self, seqs, t_list):
        """Closed-form update of P tensor via flattening into linear system."""
        dim = self.m*self.m*self.o
                            ...
                            for i2 in range(self.m):
                                for j2 in range(self.m):
                                    for h in range(self.o):
                                        idx2 = ((i2*self.m + j2)*self.o + h)
                                        per2 = self.periods[i2][j2][h]
                                        phi2 = math.cos(2*math.pi*k/per2)
                                        b = x[j2]*phi2
                                        U[idx1][idx2] += a*b

        U_inv = self._invert(U)
        sol = [sum(U_inv[r][c]*V[c] for c in range(dim)) for r in range(dim)]
        # unpack back into self.P
        for i in range(self.m):
            for j in range(self.m):
                for g in range(self.o):
                    idx = ((i*self.m + j)*self.o + g)
                    self.P[i][j][g] = sol[idx]

    def update_x(self, seqs, t_list):
        """Closed-form update of each x_map[token]."""
        for c in self.tokens:
            M = [[0.0]*self.m for _ in range(self.m)]
            R = [0.0]*self.m
            cnt=0
            for seq, t in zip(seqs, t_list):
                toks = self.extract_tokens(seq)
                    ...
                    for i in range(self.m):
                        for j in range(self.m):
                            ...
                            for g in range(self.o):
                                period = self.periods[i][j][g]
                                phi = math.cos(2*math.pi*k/period)
                                # inner prod of P[i,:,g] and P[j,:,g]
                                ip = sum(self.P[i][q][g]*self.P[j][q][g]
                        ...
                        s2=0.0
                        for g in range(self.o):
                            period = self.periods[i][j][g] if j<self.m else 1
                            phi = math.cos(2*math.pi*k/period)
                            ip2 = sum(self.P[i][q][g]*t[q] for q in range(self.m))
                            s2 += phi*ip2
                        R[i] += s2
                    cnt+=1
            if cnt>0:
    ...
    def auto_train(self, seqs, max_iters=100, tol=1e-6, learning_rate=0.01, continued=False, auto_mode='reg'):
        """
        ...
        for it in range(max_iters):
            ...
            print(f"AutoTrain({mode_display}) Iter {it:3d}: loss = {avg_loss:.6f}")
    ...
    def part_train(self, vec_seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
        ...
        for it in range(max_iters):
            ...
            history.append(avg_loss)
            mode_display = "Gap" if auto_mode == 'gap' else "Reg"
            print(f"PartTrain({mode_display}) Iter {it:3d}: loss = {avg_loss:.6f}")
...
if __name__=="__main__":
    ...
    dd = DualDescriptorTSR(charset, rank=3, vec_dim=2, num_basis=5, mode='nonlinear', user_step=2)
    ...
    # Initialize model
    dd_grad = DualDescriptorTSR(charset, rank=3, vec_dim=2, num_basis=5, mode='nonlinear', user_step=2)
...
# === Combined Auto-Training Example ===
    ...
    # Create new models
    dd_gap = DualDescriptorTSR(charset, rank=3, vec_dim=2, num_basis=5, mode='nonlinear', user_step=2)
    dd_reg = DualDescriptorTSR(charset, rank=3, vec_dim=2, num_basis=5, mode='nonlinear', user_step=2)
    ...
    gap_history = dd_gap.auto_train(auto_seqs, max_iters=500, tol=1e-8, learning_rate=0.5, continued=False, auto_mode='gap')
    
    # Perform auto-regressive training (reg mode)
    print("\nTraining in 'reg' mode (next-token prediction):")
    reg_history = dd_reg.auto_train(auto_seqs, max_iters=500, tol=1e-8, learning_rate=0.5, continued=False, auto_mode='reg')
    ...
    # Load saved model
    dd_loaded = DualDescriptorTSR.load("trained_model.pkl")
    ...
    # Create new model
    dd_part = DualDescriptorTSR(charset="", rank=3, vec_dim=2, num_basis=5)
    ...
    # Create new model
    dd_double = DualDescriptorTSR(charset=['A','C','G','T'], 
                                     rank=3, 
                                     vec_dim=2, 
                                     num_basis=5,
                                     mode='nonlinear',
