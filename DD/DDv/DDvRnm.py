...
class DualDescriptorRNM:
    ...
    def __init__(self, charset, rank=1, vec_dim=4, mode='linear', user_step=None):
        ...
        self.Bbasis = None   # L×m (now trainable)
    ...
    def grad_train(self, seqs, t_list, learning_rate=0.01, max_iters=100, tol=1e-6, print_every=10):
        ...
        2. For each iteration:
            ...
            b. Update all parameters using gradient descent
        ...
        for it in range(max_iters):
            ...
            for j in range(self.L):
                for i in range(self.m):
                    self.Bbasis[j][i] -= learning_rate * grad_B[j][i] / total_positions
    ...
    def auto_train(self, seqs, learning_rate=0.01, max_iters=100, tol=1e-6, 
        ...
        for it in range(max_iters):
            ...
            if total_instances > 0:
                ...
                for j in range(self.L):
                    for i in range(self.m):
                        self.Bbasis[j][i] -= learning_rate * grad_B[j][i] / total_instances
    ...
    # ---- show state ----
    def show(self):
        print("DualDescriptorRNM status:")
    ...
    def part_train(self, vec_seqs, learning_rate=0.01, max_iters=100, tol=1e-6, 
        ...
        # Initialize AcoeffI (m×L) with random values
        self.AcoeffI = [[random.uniform(-0.1, 0.1) for _ in range(self.L)] 
                        for _ in range(self.m)]
        
        # Initialize trainable BbasisI (L×m) with random values
        ...
        for it in range(max_iters):
            ...
            grad_BbasisI = [[0.0] * self.m for _ in range(self.L)]
            ...
            if total_instances > 0:
                ...
                for j in range(self.L):
                    for i in range(self.m):
                        self.BbasisI[j][i] -= learning_rate * grad_BbasisI[j][i] / total_instances
    ...
    def save(self, filename):
        """
        Save the current state of the DualDescriptorRNM object to a binary file.
        """
    ...
    def load(cls, filename):
        """
        Load a saved DualDescriptorRNM model from a binary file.
        """
...
if __name__ == "__main__":
    ...
    #random.seed(0)
    charset = ['A','C','G','T']
    dd = DualDescriptorRNM(charset, rank=3, vec_dim=2, mode='nonlinear', user_step=2)
...
##    print(f"Stochastic (tau=0.5): {seq_stoch[:50]}...")
    ...
    dd_grad = DualDescriptorRNM(charset, rank=3, vec_dim=2, mode='nonlinear', user_step=2)
    ...
    # Create models
    dd_gap = DualDescriptorRNM(charset, rank=2, vec_dim=3, mode='linear')
    dd_reg = DualDescriptorRNM(charset, rank=2, vec_dim=3, mode='linear')
    ...
    # Create a DualDescriptorRNM instance
    dd = DualDescriptorRNM(charset="ACGT", rank=3, vec_dim=3)
    ...
    # Load saved model
    dd_loaded = DualDescriptorRNM.load("trained_model.pkl")
