...
class NumDualDescriptorPM(nn.Module):
    ...
    def _precompute_phi_table(self, max_k):
        ...
        phi_table = torch.zeros(max_k, self.vec_dim, self.vec_dim, device=self.device, dtype=torch.float32)
    ...
    def _compute_Nk_from_x(self, k_tensor, x):
        ...
        # Basis functions: cos(2π * k / periods) [batch_size, vec_dim, vec_dim]
        phi = torch.cos(2 * math.pi * k_expanded / self.periods)
        # Nk = sum_{i,j} P[i,j] * x_j * phi_{i,j}(k)
    ...
    def reg_train(self, vector_seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        # Maximum k after extraction: number of windows in a sequence
        if self.mode == 'linear':
...
if __name__=="__main__":
    ...
    # Generate sample sequences (ensure equal length for fast path)
    self_seqs = []
    fixed_len = 200  # Use fixed length to demonstrate batch acceleration
    for _ in range(10):
