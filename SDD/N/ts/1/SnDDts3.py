...
class SpatialNumDualDescriptorTS3(nn.Module):
    ...
    def _aggregate_window(self, window_vectors):
        ...
        window_vectors: shape [batch, n_windows, rank_h*rank_w*rank_d, m]
    ...
    def batch_represent(self, batch_grids):
        ...
        Assumes all grids have the same dimensions (H, W, D) and rank_mode == 'drop'.

        Args:
            batch_grids: Tensor of shape [batch_size, H, W, D, vec_dim]

        Returns:
            representations: Tensor of shape [batch_size, vec_dim]
        """
        batch_grids = batch_grids.to(self.device)
        batch_size, H, W, D, m = batch_grids.shape
        ...
        if not row_starts or not col_starts or not depth_starts:
            return torch.zeros(batch_size, m, device=self.device)

        n_rows = len(row_starts)
        n_cols = len(col_starts)
        n_depths = len(depth_starts)
        n_windows = n_rows * n_cols * n_depths

        # Permute to [batch, channels, height, width, depth]
        x = batch_grids.permute(0, 4, 1, 2, 3)  # [batch, m, H, W, D]
        ...
        # Extract windows using depth loop + 2D unfold
        all_windows = []  # list of [batch, n_rows*n_cols, rank_h*rank_w*rank_d, m]
        for k_start in depth_starts:
            # Extract depth block of size rank_d
            if k_start + self.rank_d > D:
                if self.rank_mode == 'pad':
                    frag_d = x[:, :, :, :, k_start:D]
                    pad_d = self.rank_d - frag_d.shape[4]
                    padding = torch.zeros(frag_d.shape[0], frag_d.shape[1], frag_d.shape[2], frag_d.shape[3], pad_d, device=self.device)
                    frag_d = torch.cat([frag_d, padding], dim=4)
                else:
                    continue
            else:
                frag_d = x[:, :, :, :, k_start:k_start+self.rank_d]  # [batch, m, H, W, rank_d]

            # Combine depth dimension with channels: [batch, m*rank_d, H, W]
            batch, c, h, w, d = frag_d.shape
            frag_2d = frag_d.permute(0, 1, 4, 2, 3).reshape(batch, c * d, h, w)
            ...
            )  # [batch, (m*rank_d)*rank_h*rank_w, n_windows_hw]

            n_windows_hw = windows_2d.shape[2]
            # Reshape to [batch, n_windows_hw, rank_h, rank_w, rank_d, m]
            windows_2d = windows_2d.view(batch, m, self.rank_d, self.rank_h, self.rank_w, n_windows_hw).permute(0, 5, 3, 4, 2, 1)
            windows_flat = windows_2d.reshape(batch, n_windows_hw, self.rank_h * self.rank_w * self.rank_d, m)
            all_windows.append(windows_flat)

        if not all_windows:
            return torch.zeros(batch_size, m, device=self.device)

        # Concatenate windows from all depth starts: [batch, n_windows, rank_h*rank_w*rank_d, m]
        windows_all = torch.cat(all_windows, dim=1)  # n_windows = n_rows * n_cols * n_depths
        ...
        # Order: for each depth_start, for each row_start, for each col_start
        # This matches the extraction order (depth outer loop, then row, then col)
        k1_list = []
        k2_list = []
        k3_list = []
        for k in depth_starts:
            for i in row_starts:
                for j in col_starts:
                    k1_list.append(i)
                    k2_list.append(j)
        ...
        Nk_flat = self.batch_compute_Nk(k1_flat, k2_flat, k3_flat, agg_flat)  # [batch*n_windows, m]
    ...
    def batch_compute_Nk_and_targets(self, batch_grids):
        ...
        Args:
            batch_grids: Tensor of shape [batch_size, H, W, D, m]
        ...
        # Extract windows using depth loop + 2D unfold (same as batch_represent)
        all_windows = []
        for k_start in depth_starts:
            if k_start + self.rank_d > D:
                if self.rank_mode == 'pad':
                ...
                frag_d = x[:, :, :, :, k_start:k_start+self.rank_d]
        ...
        windows_all = torch.cat(all_windows, dim=1)  # [batch, n_windows, n_vecs, m]
        ...
        k2_flat = k2_expanded.reshape(batch_size * n_windows)
        k3_flat = k3_expanded.reshape(batch_size * n_windows)
        Nk_flat = self.batch_compute_Nk(k1_flat, k2_flat, k3_flat, agg_flat2)
    ...
    def batch_compute_Nk(self, k1_tensor, k2_tensor, k3_tensor, vectors):
        ...
        # Calculate basis functions for three dimensions
    ...
    def reg_train(self, arrays, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                shapes = [arr.shape if isinstance(arr, torch.Tensor) else (len(arr), len(arr[0]), len(arr[0][0]), self.vec_dim) for arr in batch_arrays]
    ...
    def cls_train(self, arrays, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
                  continued=False, decay_rate=1.0, print_every=10, batch_size=32,
                  checkpoint_file=None, checkpoint_interval=10):
        """Train for multi-class classification on 3D arrays with batch acceleration."""
    ...
    def lbl_train(self, arrays, labels, num_labels, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        """Train for multi-label classification on 3D arrays with batch acceleration."""
    ...
    def self_train(self, arrays, max_iters=100, tol=1e-6, learning_rate=0.01,
        ...
        """
        Self-training for self-consistency on 3D arrays with batch acceleration.
...
if __name__ == "__main__":
    ...
    print("="*50)
    print("Spatial Numerical Dual Descriptor TS3 - 3D Array Version with Batch Acceleration")
