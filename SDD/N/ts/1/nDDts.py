...
class NumDualDescriptorTS(nn.Module):
    ...
    def batch_compute_Nk(self, k_tensor, vectors):
        ...
        return self._compute_Nk_from_x(k_tensor, x)
    ...
    def self_train(self, vector_seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
        ...
        seq_lengths = [seq.shape[0] for seq in vector_seqs]
        ...
        if use_fast_batch:
            ...
            # Process sequences individually (original method)
            for it in range(max_iters):
                total_loss = 0.0
                total_samples = 0
...
if __name__=="__main__":
    ...
    # Generate sample sequences with equal length to demonstrate fast batch path
    fixed_len = 200
    self_seqs = []
    for _ in range(10):
        self_seqs.append(torch.randn(fixed_len, vec_dim))
    
    # Conduct self-consistency training (will use fast batch processing because all sequences have equal length)
