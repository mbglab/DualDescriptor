...
# The Numerical Dual Descriptor Vector class (Tensor form) implemented with PyTorch
...
class NumDualDescriptorTS(nn.Module):
    ...
    Numerical Vector Dual Descriptor with GPU acceleration using PyTorch:
      ...
      - batch acceleration for equal-length sequences when rank_mode == 'drop'
    """
    def __init__(self, vec_dim, rank=1, rank_op='avg', rank_mode='drop', num_basis=5, mode='linear', user_step=None, device='cuda'):
        ...
        # Precomputed phi table for all possible k values (initially None, built when needed)
    ...
    def _precompute_phi_table(self, max_k):
        """Precompute phi values for k = 0, 1, ..., max_k-1 and store as buffer.
        This significantly speeds up batch_compute_Nk by avoiding repeated trig calls.
        """
        if self.phi_table is not None and self.phi_table.size(0) >= max_k:
            return  # already have enough precomputed values
        # Allocate and compute
        phi_table = torch.zeros(max_k, self.m, self.m, self.o, device=self.device, dtype=torch.float32)
        for k in range(max_k):
            phi_table[k] = torch.cos(2 * math.pi * k / self.periods)
    ...
    def _compute_Nk_from_x(self, k_tensor, x):
        """
        Compute N(k) vectors directly from mapped vectors x (already transformed by M).
        This avoids double mapping in batch processing.
        
        Args:
            k_tensor: Tensor of position indices [batch_size] (float)
        ...
        # Basis functions: cos(2π * k / periods) [batch_size, m, m, o]
        phi = torch.cos(2 * math.pi * k_expanded / self.periods)
        # Nk = sum_{i,j,g} P[i,j,g] * x_j * phi_{i,j,g}(k)
    ...
    def extract_vectors(self, seq_vectors):
        ...
        Returns:
            torch.Tensor: Tensor of vectors after applying rank operation to each extracted vector group
        """
        L = len(seq_vectors)
    ...
    def batch_compute_Nk(self, k_tensor, vectors):
        ...
        Args:
            k_tensor: Tensor of position indices [batch_size] (should be integer)
        ...
        # Convert k_tensor to integer indices (safe for indexing)
        k_idx = k_tensor.long()
        
        # Use precomputed phi table
        if self.phi_table is None or k_idx.max().item() >= self.phi_table.size(0):
            raise RuntimeError("phi_table not precomputed for required k range. "
                               "Please call _precompute_phi_table(max_k) before training.")
        
        # phi: [batch_size, m, m, o]
        phi = self.phi_table[k_idx]
    ...
    def batch_represent(self, seq_batch):
        ...
        in the batch have identical length and rank_mode == 'drop'.
        
        Args:
            seq_batch: Tensor of shape [batch_size, seq_len, vec_dim]
        ...
        # Permute to [batch, m, seq_len] for unfold
        x = seq_batch.permute(0, 2, 1)  # [batch, m, seq_len]
        
        # Pad if necessary (for 'pad' mode) – but this function is only used when rank_mode == 'drop'
        # However, we keep the logic for completeness (though it won't be used in fast path).
        ...
        mapped = self.M(windows)  # [batch, n_windows, rank, m]
        ...
        # Now compute Nk for each window using the already mapped and aggregated vectors
        # Build k tensor for each window position
        k_tensor = torch.tensor(starts, dtype=torch.float32, device=self.device)  # [n_windows]
        # Expand to batch dimension
        k_expanded = k_tensor.unsqueeze(0).expand(batch_size, -1)  # [batch, n_windows]
        # Flatten to compute all Nk in one go
        agg_flat = agg.reshape(batch_size * n_windows, m)   # [batch*n_windows, m]
        k_flat = k_expanded.reshape(batch_size * n_windows)  # [batch*n_windows]
        
        # Compute Nk for all windows using _compute_Nk_from_x (no extra M mapping)
        Nk_flat = self._compute_Nk_from_x(k_flat, agg_flat)  # [batch*n_windows, m]
        Nk = Nk_flat.view(batch_size, n_windows, m)  # [batch, n_windows, m]
        
        # Average over windows to get sequence representation
    ...
    def batch_compute_Nk_and_targets(self, seq_batch):
        ...
        # Compute Nk using _compute_Nk_from_x
        k_tensor = torch.tensor(starts, dtype=torch.float32, device=self.device)  # [n_windows]
        k_expanded = k_tensor.unsqueeze(0).expand(batch_size, -1)  # [batch, n_windows]
        targets_flat = targets.reshape(batch_size * n_windows, m)  # [batch*n_windows, m]
        k_flat = k_expanded.reshape(batch_size * n_windows)        # [batch*n_windows]
        Nk_flat = self._compute_Nk_from_x(k_flat, targets_flat)    # [batch*n_windows, m]
        Nk_all = Nk_flat.view(batch_size, n_windows, m)            # [batch, n_windows, m]
    ...
    def compute_Nk(self, k, vector):
        ...
        k_tensor = torch.tensor([k], dtype=torch.long, device=self.device)
    ...
    def describe(self, vectors):
        ...
        k_positions = torch.arange(extracted_vectors.shape[0], dtype=torch.long, device=self.device)
    ...
    def reg_train(self, vector_seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        Optimized for GPU memory efficiency by processing sequences individually, but
        uses fast batch processing when all sequences in a batch have equal length
        and rank_mode == 'drop'.
        ...
        # Ensure all sequences are on the correct device and are tensors
        vector_seqs = [seq.to(self.device) if isinstance(seq, torch.Tensor) else 
                       torch.tensor(seq, dtype=torch.float32, device=self.device) 
                       for seq in vector_seqs]
        
        # Precompute phi table based on max possible k (max sequence length after extraction)
        max_seq_len = max(len(seq) for seq in vector_seqs)
        if self.mode == 'linear':
            max_windows = max_seq_len - self.rank + 1
        else:
            step = self.step or self.rank
            max_windows = (max_seq_len + step - 1) // step  # rough upper bound
        self._precompute_phi_table(max_windows)
        
        # Pre-extract window vectors for all sequences (speed optimization)
        extracted_vectors_list = []
        for seq in vector_seqs:
            extracted = self.extract_vectors(seq)
            extracted_vectors_list.append(extracted)
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                batch_extracted = [extracted_vectors_list[idx] for idx in batch_indices]
                ...
                if use_fast_batch and len(set(seq_lengths)) == 1:
                    ...
                    # Process each sequence individually using pre-extracted vectors
                    for extracted_vectors, target in zip(batch_extracted, batch_targets):
                        if extracted_vectors.shape[0] == 0:
                            continue
    ...
    def cls_train(self, vector_seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        if self.mode == 'linear':
            ...
            step = self.step or self.rank
            max_windows = (max_seq_len + step - 1) // step
        self._precompute_phi_table(max_windows)
        
        # Pre-extract window vectors for all sequences
        extracted_vectors_list = []
        for seq in vector_seqs:
            extracted = self.extract_vectors(seq)
            extracted_vectors_list.append(extracted)
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                if use_fast_batch and len(set(seq_lengths)) == 1:
                    ...
                    # Process each sequence individually using pre-extracted vectors
                    batch_logits = []
                    for extracted_vectors in batch_extracted:
                        if extracted_vectors.shape[0] == 0:
    ...
    def self_train(self, vector_seqs, max_iters=100, tol=1e-6, learning_rate=0.01, 
        ...
        Uses fast batch processing when all sequences have equal length and rank_mode == 'drop'.
        ...
        seq_lengths = [len(seq) for seq in vector_seqs]
        ...
        if use_fast_batch:
            ...
            all_seqs = torch.stack(vector_seqs, dim=0)  # [num_seqs, seq_len, m]
            num_seqs = len(vector_seqs)
            ...
            # Process sequences individually using pre-extracted vectors
            for it in range(max_iters):
                total_loss = 0.0
                total_samples = 0
                ...
                for batch_start in range(0, len(indices), batch_size):
                    batch_indices = indices[batch_start:batch_start + batch_size]
                    batch_seqs_extracted = [extracted_vectors_list[idx] for idx in batch_indices]
                    
                    optimizer.zero_grad()
                    batch_loss = 0.0
                    batch_sample_count = 0
                    
                    # Process each sequence in the batch using pre-extracted vectors
                    for extracted_vectors in batch_seqs_extracted:
                        if extracted_vectors.shape[0] == 0:
                            continue
                        ...
                        del Nk_batch, target_vectors, k_positions
    ...
    def predict_t(self, vectors):
        ...
        # Use batch_represent for efficiency (single sequence)
        if not isinstance(vectors, torch.Tensor):
            vectors = torch.tensor(vectors, dtype=torch.float32, device=self.device)
        elif vectors.device != self.device:
            vectors = vectors.to(self.device)
        
        # Add batch dimension and call batch_represent
        seq_batch = vectors.unsqueeze(0)  # [1, seq_len, m]
        rep = self.batch_represent(seq_batch)  # [1, m]
    ...
    def reconstruct(self, L, tau=0.0):
        ...
        for k in range(num_windows):
            ...
            k_tensor = torch.tensor([k] * num_candidates, dtype=torch.long, device=self.device)
...
if __name__=="__main__":
    ...
    print("="*50)
    print("Numerical Dual Descriptor TS - PyTorch GPU Accelerated Version")
    print("Processes sequences of m-dimensional real vectors")
    print("="*50)
    ...
    # Parameters
    vec_dim = 5        # Dimension of input vectors and internal representation
    ...
    # Generate sample sequences (ensure equal length for fast path demonstration)
    fixed_len = 200
    self_seqs = []
    for _ in range(10):
        self_seqs.append(torch.randn(fixed_len, vec_dim))
    
    # Conduct self-consistency training (will use fast batch processing)
    print("\nTraining for self-consistency (fast batch mode):")
