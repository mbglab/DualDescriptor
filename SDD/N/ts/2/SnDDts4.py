...
class SpatialNumDualDescriptorTS4(nn.Module):
    ...
    def __init__(self, vec_dim, rank=1, rank_op='avg', rank_mode='drop', num_basis=5, mode='linear',
        ...
        self.register_buffer('phi4_table', None)
        ...
        # Cache for window indices for batch acceleration (key: (H,W,D,T) tuple)
    ...
    def _precompute_phi_tables(self, max_k1, max_k2, max_k3, max_k4):
        """
        Precompute phi tables for all possible k1, k2, k3, k4 values.
        ...
        if (self.phi1_table is not None and self.phi2_table is not None and
            self.phi3_table is not None and self.phi4_table is not None and
            self.phi1_table.size(0) >= max_k1 and self.phi2_table.size(0) >= max_k2 and
            self.phi3_table.size(0) >= max_k3 and self.phi4_table.size(0) >= max_k4):
            return
        ...
        # Phi4 table
        phi4 = torch.zeros(max_k4, self.m, self.m, self.o, device=self.device, dtype=torch.float32)
        for k in range(max_k4):
            phi4[k] = torch.cos(2 * math.pi * k / self.periods4)
        self.register_buffer('phi4_table', phi4)

    def _compute_window_indices_4d(self, H, W, D, T):
        """
        Precompute linear indices for all windows in a 4D grid of given dimensions.
        ...
        Returns:
            flat_indices: torch.LongTensor of shape [num_windows * window_size] with linear indices
                          into the flattened grid (row-major: H first, then W, then D, then T).
            ...
            k4_list: list of starting time indices
            num_windows: int
            window_size: int
        """
        if (H, W, D, T) in self._window_index_cache:
            cached = self._window_index_cache[(H, W, D, T)]
            return cached['flat_indices'], cached['k1_list'], cached['k2_list'], cached['k3_list'], cached['k4_list'], cached['num_windows'], cached['window_size']
        ...
        k4_starts = list(range(0, T - self.rank_t + 1, step_t))
        
        if not k1_starts or not k2_starts or not k3_starts or not k4_starts:
            cache = {'flat_indices': None, 'k1_list': [], 'k2_list': [], 'k3_list': [], 'k4_list': [], 'num_windows': 0, 'window_size': 0}
            self._window_index_cache[(H, W, D, T)] = cache
            return None, [], [], [], [], 0, 0
        
        num_windows = len(k1_starts) * len(k2_starts) * len(k3_starts) * len(k4_starts)
        window_size = self.rank_h * self.rank_w * self.rank_d * self.rank_t
        ...
        # Linear index function: idx = i*W*D*T + j*D*T + k*T + t
        stride_wdt = W * D * T
        stride_dt = D * T
        stride_t = T
                            ...
                            for dj in range(self.rank_w):
                                for dk in range(self.rank_d):
                                    for dt in range(self.rank_t):
                                        linear = (i+di) * stride_wdt + (j+dj) * stride_dt + (k+dk) * stride_t + (t+dt)
        ...
        self._window_index_cache[(H, W, D, T)] = cache
        return flat_indices, k1_list, k2_list, k3_list, k4_list, num_windows, window_size

    def _compute_Nk_from_mapped(self, k1_tensor, k2_tensor, k3_tensor, k4_tensor, x):
        """
        ...
        Args:
            ...
            k4_tensor: Tensor of time indices [batch_size] (long)
        ...
        if (self.phi1_table is not None and self.phi2_table is not None and
            ...
            k3_idx.max().item() < self.phi3_table.size(0) and
            k4_idx.max().item() < self.phi4_table.size(0)):
            phi1 = self.phi1_table[k1_idx]  # [batch, m, m, o]
            phi2 = self.phi2_table[k2_idx]
            phi3 = self.phi3_table[k3_idx]
            phi4 = self.phi4_table[k4_idx]
    ...
    def batch_represent_4d(self, grid_batch):
        """
        ...
        identical dimensions (H, W, D, T) and rank_mode == 'drop'.
        
        Args:
            grid_batch: Tensor of shape [batch_size, H, W, D, T, m]
        
        Returns:
            representations: Tensor of shape [batch_size, m]
        """
        grid_batch = grid_batch.to(self.device)
        batch_size, H, W, D, T, m = grid_batch.shape
        assert self.rank_mode == 'drop', "Batch acceleration only works with rank_mode='drop'"
        
        # Get precomputed window indices
        flat_indices, k1_list, k2_list, k3_list, k4_list, num_windows, window_size = self._compute_window_indices_4d(H, W, D, T)
        if flat_indices is None:
            return torch.zeros(batch_size, m, device=self.device)
        
        # Flatten the spatial dimensions
        flat_grid = grid_batch.view(batch_size, H * W * D * T, m)  # [batch, total_pos, m]
        ...
        # Prepare k1, k2, k3, k4 tensors for all windows
        ...
        k4_tensor = torch.tensor(k4_list, dtype=torch.long, device=self.device)
        ...
        k4_flat = k4_exp.reshape(batch_size * num_windows)
        
        # Compute Nk for all windows
        Nk_flat = self._compute_Nk_from_mapped(k1_flat, k2_flat, k3_flat, k4_flat, agg_flat)
    ...
    def batch_compute_Nk_and_targets_4d(self, grid_batch):
        """
        ...
        flat_grid = grid_batch.view(batch_size, H * W * D * T, m)
        ...
        Nk_flat = self._compute_Nk_from_mapped(k1_flat, k2_flat, k3_flat, k4_flat, targets_flat)
    ...
    def batch_compute_Nk(self, k1_tensor, k2_tensor, k3_tensor, k4_tensor, vectors):
        ...
        # Try to use precomputed phi tables if available and sufficient
        use_precomputed = (self.phi1_table is not None and self.phi2_table is not None and
                           self.phi3_table is not None and self.phi4_table is not None and
                           k1_idx.max().item() < self.phi1_table.size(0) and
                           k2_idx.max().item() < self.phi2_table.size(0) and
                           k3_idx.max().item() < self.phi3_table.size(0) and
    ...
    def reg_train(self, arrays, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        max_T = max(arr.shape[3] for arr in arrays)
        self._precompute_phi_tables(max_H, max_W, max_D, max_T)
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                grid_shapes = [(arr.shape[0], arr.shape[1], arr.shape[2], arr.shape[3]) for arr in batch_arrays]
                if use_fast_batch and len(set(grid_shapes)) == 1:
                    ...
                    batch_grid_tensor = torch.stack(batch_arrays_on_device, dim=0)  # [batch, H, W, D, T, m]
                    # Compute grid representations
                    reps = self.batch_represent_4d(batch_grid_tensor)  # [batch, m]
    ...
    def cls_train(self, arrays, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
                  continued=False, decay_rate=1.0, print_every=10, batch_size=32,
                  checkpoint_file=None, checkpoint_interval=10):
        """Train for multi-class classification on 4D arrays with optional batch acceleration."""
    ...
    def lbl_train(self, arrays, labels, num_labels, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        """Train for multi-label classification on 4D arrays with optional batch acceleration."""
    ...
    def self_train(self, arrays, max_iters=100, tol=1e-6, learning_rate=0.01,
        ...
        """
        Self-training for self-consistency on 4D arrays.
        ...
        grid_shapes = [(arr.shape[0], arr.shape[1], arr.shape[2], arr.shape[3]) for arr in arrays]
        ...
        if use_fast_batch:
            ...
            for it in range(max_iters):
                ...
                for batch_start in range(0, num_grids, seq_batch_size):
                    ...
                    batch_grids = all_grids[batch_indices]  # [batch, H, W, D, T, m]
                    
                    optimizer.zero_grad()
                    
                    # Compute Nk and targets for all windows in this batch of grids
                    Nk_batch, targets_batch = self.batch_compute_Nk_and_targets_4d(batch_grids)
...
if __name__ == "__main__":
    ...
    print("="*50)
    print("Spatial Numerical Dual Descriptor TS4 - 4D Array Version (Accelerated)")
