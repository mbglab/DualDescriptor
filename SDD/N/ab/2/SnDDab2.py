...
class SpatialNumDualDescriptorAB2(nn.Module):
    ...
    def extract_vectors(self, vec_arr):
        ...
        Returns:
            tuple: (list of aggregated vectors, list of window coordinates as tuples (k1, k2))
        """
        ...
        H, W, m = vec_arr.shape
        if H < self.rank1 or W < self.rank2:
            return [], []
        
        # Use vectorized approach for linear mode with avg/sum
        if self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
            # Prepare for unfold: shape (1, m, H, W)
            seq_reshaped = vec_arr.permute(2, 0, 1).unsqueeze(0)  # (1, m, H, W)
            # Unfold to extract windows
            windows = F.unfold(seq_reshaped, kernel_size=(self.rank1, self.rank2), stride=1)
            # windows shape: (1, m * rank1 * rank2, num_windows)
            num_windows = windows.shape[2]
            # Reshape to (num_windows, rank1 * rank2, m)
            windows = windows.view(1, self.m, self.rank1 * self.rank2, num_windows).squeeze(0)  # (m, rank1*rank2, num_windows)
            windows = windows.permute(2, 1, 0)  # (num_windows, rank1*rank2, m)
            
            # Aggregate along window elements
            if self.rank_op == 'avg':
            ...
            # Generate coordinates for each window (top-left corner)
            k1_list = []
            k2_list = []
            ...
            # Return as list of tensors and coordinates
            vectors = [transformed[i] for i in range(num_windows)]
            coords = list(zip(k1_list, k2_list))
    ...
    def _get_window_data(self, vec_arr):
        """
        Internal helper: returns window aggregated vectors and their coordinates (k1, k2).
        Optimized version using vectorized operations when possible.
        
        Args:
            vec_arr: tensor of shape (H, W, m)
        ...
        if not agg_list:
            return torch.empty(0, self.m, device=self.device), torch.empty(0, device=self.device), torch.empty(0, device=self.device)
        ...
        return agg_tensor, k1_tensor, k2_tensor

    def _batch_window_data_2d(self, arr_batch):
        """
        Efficiently compute aggregated window vectors and their positions for a batch of 2D arrays.
        Assumes all arrays have the same shape (H, W, m) and conditions for vectorized extraction are met:
            - mode == 'linear'
            - rank_op in ('avg', 'sum')
            - H >= rank1 and W >= rank2
        ...
        Args:
            arr_batch: torch.Tensor of shape [B, H, W, m]
        ...
        # Check conditions
        if self.mode != 'linear' or self.rank_op not in ('avg', 'sum') or H < self.rank1 or W < self.rank2:
            return None
        
        # Prepare for unfold: [B, m, H, W]
        seq_reshaped = arr_batch.permute(0, 3, 1, 2)  # [B, m, H, W]
        # Unfold: kernel size (rank1, rank2), stride=1
        windows = F.unfold(seq_reshaped, kernel_size=(self.rank1, self.rank2), stride=1)
        # windows shape: [B, m * rank1 * rank2, num_windows]
        num_windows = windows.shape[2]  # per array
        # Reshape to [B, num_windows, rank1*rank2, m]
        windows = windows.view(B, self.m, self.rank1 * self.rank2, num_windows).permute(0, 3, 2, 1)
        # Aggregate
        if self.rank_op == 'avg':
            agg = windows.mean(dim=2)   # [B, num_windows, m]
        else:  # sum
            agg = windows.sum(dim=2)    # [B, num_windows, m]
        # Apply M
        ...
        # Generate coordinates for each window (top-left corner) for a single array
        k1 = torch.arange(H - self.rank1 + 1, device=self.device).repeat_interleave(W - self.rank2 + 1)
        k2 = torch.arange(W - self.rank2 + 1, device=self.device).repeat(H - self.rank1 + 1)
        # Repeat for batch
        k1_flat = k1.repeat(B)
        k2_flat = k2.repeat(B)
        window_counts = torch.full((B,), num_windows, dtype=torch.long, device=self.device)
        
        return agg_flat, k1_flat, k2_flat, window_counts

    def describe(self, vec_arr):
        """Compute N(k1,k2) vectors for each window in the 2D vector array"""
        agg_tensor, k1_tensor, k2_tensor = self._get_window_data(vec_arr)
        if agg_tensor.shape[0] == 0:
            return []
        
        j1_t = k1_tensor % self.L1
        j2_t = k2_tensor % self.L2
    ...
    def reg_train(self, vec_arrs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        for it in range(start_iter, max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                shapes = [(arr.shape[0], arr.shape[1]) for arr in batch_arrs]
                if len(set(shapes)) == 1 and self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
                    # Equal shape: use vectorized batch extraction
                    H, W = shapes[0]
                    if H >= self.rank1 and W >= self.rank2:
                        # Stack into a single tensor [B, H, W, m]
                        batch_tensor = torch.stack(batch_arrs, dim=0)
                        result = self._batch_window_data_2d(batch_tensor)
                        if result is not None:
                            agg_flat, k1_flat, k2_flat, window_counts = result
                            ...
                            # Cleanup
                            del batch_tensor, agg_flat, k1_flat, k2_flat, window_counts, Nk_all, array_reps, reps_tensor
                ...
                for arr in batch_arrs:
                    agg, k1, k2 = self._get_window_data(arr)
                    if agg.shape[0] == 0:
                ...
                if len(set(shapes)) == 1 and self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
                    ...
                    if H >= self.rank1 and W >= self.rank2:
                        ...
                        if result is not None:
                            ...
                            # Cleanup
                            del batch_tensor, agg_flat, k1_flat, k2_flat, window_counts, Nk_all
                            if torch.cuda.is_available():
                                torch.cuda.empty_cache()
                            continue  # skip fallback
    ...
    def predict_t(self, vec_arr):
        ...
        agg, k1, k2 = self._get_window_data(vec_arr)
        if agg.shape[0] == 0:
