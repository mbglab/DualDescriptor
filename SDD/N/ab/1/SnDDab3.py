...
class SpatialNumDualDescriptorAB3(nn.Module):
    ...
    def extract_vectors(self, vec_arr):
        ...
        D, H, W, _ = vec_arr.shape  # depth, height, width, vector dimension

        # Helper function to apply vector operations to a window
        def apply_op(window):
            """
            Apply mapping and aggregation to a window of vectors.
            window shape: [rank1, rank2, rank3, m]
            """
            # Flatten window to a list of vectors
            flat_window = window.reshape(-1, self.m)  # [rank1*rank2*rank3, m]
            # Apply mapping matrix M to each vector
            transformed = torch.matmul(flat_window, self.M.T)  # [rank1*rank2*rank3, m]
    ...
    # ----------------------------------------------------------------------
    #  Batch acceleration methods (inspired by SnDDab2.py)
    # ----------------------------------------------------------------------
    def _get_window_coords_and_indices_3d(self, D, H, W):
        ...
        if self.rank_mode == 'drop':
            ...
            d_range = range(0, D - self.rank1 + 1, step1)
            h_range = range(0, H - self.rank2 + 1, step2)
            w_range = range(0, W - self.rank3 + 1, step3)
        else:  # 'pad'
            # Include all windows that start before D,H,W (pad with zeros if needed)
            d_range = range(0, D, step1)
            h_range = range(0, H, step2)
            w_range = range(0, W, step3)
        ...
        for d in d_range:
            for h in h_range:
                for w in w_range:
                    coords.append((d, h, w))
                    j1_list.append(d % self.L1)
                    j2_list.append(h % self.L2)
                    j3_list.append(w % self.L3)
    ...
    def batch_represent(self, arr_batch):
        ...
        if self.rank_mode == 'pad':
            pad_d = (self.rank1 - 1) if step1 == 1 else (self.rank1 - step1)
            pad_h = (self.rank2 - 1) if step2 == 1 else (self.rank2 - step2)
            pad_w = (self.rank3 - 1) if step3 == 1 else (self.rank3 - step3)
        ...
        # Use 2D unfold on the merged spatial dimensions after handling depth
        # We will extract 3D windows by iterating over depth steps and applying 2D unfold on the
        # corresponding depth-sliced data.
        # First compute number of windows along each dimension
        n_windows_d = ((D_pad - self.rank1) // step1 + 1) if D_pad >= self.rank1 else 0
        n_windows_h = ((H_pad - self.rank2) // step2 + 1) if H_pad >= self.rank2 else 0
        n_windows_w = ((W_pad - self.rank3) // step3 + 1) if W_pad >= self.rank3 else 0
        ...
        # We'll store them in a tensor of shape [B, total_windows, m]
        # Instead of building a large list, we can allocate and fill
        agg_all = torch.zeros(B, total_windows, m, device=self.device)

        # Iterate over depth start indices
        idx_offset = 0
        for d_start in range(0, D_pad - self.rank1 + 1, step1):
            # Extract depth slice of size (rank1) for each batch
            # x_d: [B, m, rank1, H_pad, W_pad]
            x_d = x[:, :, d_start:d_start+self.rank1, :, :]
            # Merge depth into channel dimension: [B, m*rank1, H_pad, W_pad]
            x_d = x_d.contiguous().view(B, m * self.rank1, H_pad, W_pad)
            # Apply 2D unfold to get all spatial windows for this depth offset
            windows_2d = torch.nn.functional.unfold(
                x_d, kernel_size=(self.rank2, self.rank3), stride=(step2, step3)
            )  # [B, m*rank1*rank2*rank3, n_windows_h * n_windows_w]
            # Reshape to [B, n_windows_h*n_windows_w, rank1*rank2*rank3, m]
            win_size = self.rank1 * self.rank2 * self.rank3
            windows_2d = windows_2d.view(B, m, win_size, -1).permute(0, 3, 2, 1)
            # windows_2d: [B, n_spatial, win_size, m]
            # Now apply mapping matrix M
            ...
            # Store in agg_all at the appropriate offset
            n_spatial = agg.size(1)
            agg_all[:, idx_offset:idx_offset+n_spatial, :] = agg
            idx_offset += n_spatial

        # Now we have aggregated vectors for all windows: [B, total_windows, m]
        # Compute basis indices for each window
        # We can compute indices using the same method as _get_window_coords_and_indices_3d but in tensor form
        # For efficiency, we can compute the list of coordinates once and reuse for all batches
        coords, idx_basis = self._get_window_coords_and_indices_3d(D, H, W)
        if not coords:
            return torch.zeros(B, m, device=self.device)
    ...
    def batch_compute_Nk_and_targets(self, arr_batch):
        """
        Compute Nk vectors and target vectors for each position (window) in a batch of 3D arrays.
        ...
        # We'll also accumulate Nk later after we know basis indices
        idx_offset = 0
        win_size = self.rank1 * self.rank2 * self.rank3
        for d_start in range(0, D_pad - self.rank1 + 1, step1):
            x_d = x[:, :, d_start:d_start+self.rank1, :, :]  # [B, m, rank1, H_pad, W_pad]
            x_d = x_d.contiguous().view(B, m * self.rank1, H_pad, W_pad)
            windows_2d = torch.nn.functional.unfold(
                x_d, kernel_size=(self.rank2, self.rank3), stride=(step2, step3)
            )  # [B, m*rank1*rank2*rank3, n_spatial]
            windows_2d = windows_2d.view(B, m, win_size, -1).permute(0, 3, 2, 1)
            # Apply mapping
            mapped = torch.matmul(windows_2d, self.M.T)  # [B, n_spatial, win_size, m]
            if self.rank_op == 'sum':
        ...
        # Now compute basis indices for all windows
        coords, idx_basis = self._get_window_coords_and_indices_3d(D, H, W)
        if not coords:
    ...
    # ----------------------------------------------------------------------
    #  Original methods remain unchanged (describe, S, D, d, etc.)
    # ----------------------------------------------------------------------
    def describe(self, vec_arr):
        ...
        # Compute basis indices (j1, j2, j3) for each window
        j1_list = []
        j2_list = []
        j3_list = []
        for (k1, k2, k3) in coords:
            j1 = k1 % self.L1
            j2 = k2 % self.L2
            j3 = k3 % self.L3
            j1_list.append(j1)
        ...
        # Flatten index: idx = j1 * (L2*L3) + j2 * L3 + j3
        idx = (torch.tensor(j1_list, dtype=torch.long, device=self.device) * (self.L2 * self.L3) +
               torch.tensor(j2_list, dtype=torch.long, device=self.device) * self.L3 +
               torch.tensor(j3_list, dtype=torch.long, device=self.device))
        ...
        for it in range(start_iter, max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                if use_fast_batch:
                    ...
                    if len(set(shapes)) == 1:
                        H, W, D, _ = batch_arrs[0].shape
                        batch_tensor = torch.stack(batch_arrs, dim=0)  # [batch, D, H, W, m]
    ...
    def cls_train(self, vec_arrs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        Args:
            vec_arrs: List of 3D vector arrays
    ...
    def self_train(self, vec_arrs, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        if use_fast_batch:
            ...
            D, H, W, m = vec_arrs[0].shape
            num_seqs = len(vec_arrs)
            all_seqs = torch.stack(vec_arrs, dim=0)  # [num_seqs, D, H, W, m]
            ...
            for it in range(start_iter, max_iters):
                ...
                for batch_start in range(0, num_seqs, seq_batch_size):
                    ...
                    batch_seqs = all_seqs[batch_indices]  # [batch, D, H, W, m]
            ...
            for it in range(start_iter, max_iters):
                ...
                for idx in indices:
                    ...
                    idx_t = (torch.tensor(j1_list, dtype=torch.long, device=self.device) * (self.L2 * self.L3) +
                             torch.tensor(j2_list, dtype=torch.long, device=self.device) * self.L3 +
                             torch.tensor(j3_list, dtype=torch.long, device=self.device))
