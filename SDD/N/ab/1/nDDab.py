...
class NumDualDescriptorAB(nn.Module):
    ...
    def batch_represent(self, seq_batch):
        ...
        if self.rank_mode == 'pad':
            ...
            # For linear mode: step=1, need (rank-1) padding
            # For nonlinear mode: step may be >1, we need to pad such that the last start + rank <= seq_len_pad
    ...
    def reg_train(self, vec_seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        # Fast batch processing possible when rank_mode == 'drop' and all sequences in batch have same length
    ...
    def cls_train(self, vec_seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        Optimized with batch processing for sequences of equal length.
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                # Check if all sequences in this batch have the same length
                seq_lengths = [vec_seqs[idx].shape[0] for idx in batch_indices]
                if use_fast_batch and len(set(seq_lengths)) == 1:
                    # All equal: use efficient batch processing
                    batch_seqs = torch.stack([vec_seqs[idx] for idx in batch_indices], dim=0)  # [batch, seq_len, m]
                    reps = self.batch_represent(batch_seqs)  # [batch, m]
                else:
                    # Unequal lengths: fallback to original per-sequence processing
                    reps_list = []
                    for idx in batch_indices:
                        vec_seq = vec_seqs[idx]
                        transformed_vecs = self.extract_vectors(vec_seq)
                        if not transformed_vecs:
                            ...
                            scalar = torch.sum(B_rows * transformed_vecs_tensor, dim=1)
                    ...
                    reps = torch.stack(reps_list, dim=0)  # [batch, m]
    ...
    def self_train(self, vec_seqs, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        Optimized with batch processing when all sequences have equal length and rank_mode == 'drop'.
        ...
        Args:
            ...
            decay_rate: Learning rate decay rate
            print_every: Print interval
            batch_size: Number of sequences to process in each batch (for fast mode)
            checkpoint_file: Path to save/load checkpoint file for resuming training
            checkpoint_interval: Interval (in iterations) for saving checkpoints
        ...
        seq_lengths = [seq.shape[0] for seq in vec_seqs]
        ...
        if use_fast_batch:
            ...
            all_seqs = torch.stack(vec_seqs, dim=0)  # [num_seqs, seq_len, m]
            num_seqs = len(vec_seqs)
            ...
            # batch_size here is for number of samples (windows) per gradient step
    ...
    def predict_c(self, vec_seq):
        ...
        Optimized with batch processing (uses batch_represent for single sequence).

        Args:
            vec_seq: Input vector sequence (tensor or list of shape [seq_len, m])
        ...
        # Convert input to tensor on the correct device if necessary
        if not isinstance(vec_seq, torch.Tensor):
            vec_seq = torch.tensor(vec_seq, dtype=torch.float32, device=self.device)
        elif vec_seq.device != self.device:
            vec_seq = vec_seq.to(self.device)

        # Handle empty sequence (return zero vector representation)
        if vec_seq.shape[0] == 0:
            seq_vector = torch.zeros(self.m, device=self.device)
        else:
            # Add batch dimension and use the efficient batch_represent method
            # batch_represent expects shape [batch, seq_len, m]
            batch_seq = vec_seq.unsqueeze(0)          # [1, seq_len, m]
            seq_vector = self.batch_represent(batch_seq).squeeze(0)  # [m]

        # Get logits through classification head
        with torch.no_grad():
            logits = self.classifier(seq_vector.unsqueeze(0))   # [1, num_classes]
