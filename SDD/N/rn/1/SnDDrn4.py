...
class SpatialNumDualDescriptorRN4(nn.Module):
    ...
    Dual Descriptor for 4D vector spatiotemporal volumes with GPU acceleration using PyTorch:
      ...
      - Batch acceleration for equal‑shape volumes (inspired by SnDDab4.py)
    """
    ...
    # ----------------------------------------------------------------------
    #  Batch acceleration methods (inspired by SnDDab4.py)
    # ----------------------------------------------------------------------
    def _get_window_coords_and_indices(self, T, D, H, W):
        """
        Compute window top-left coordinates and corresponding composite indices for a given 4D volume size.
        Used by batch processing methods.

        Args:
            T, D, H, W: Dimensions of the volume

        Returns:
            coords: List of (t, d, h, w) tuples for each window
        ...
        if self.rank_mode == 'drop':
            ...
            range_t = range(0, T - self.rank + 1, step)
            range_d = range(0, D - self.rank + 1, step)
            range_h = range(0, H - self.rank + 1, step)
            range_w = range(0, W - self.rank + 1, step)
        else:  # 'pad'
            # Include all windows that start before T,D,H,W (pad with zeros if needed)
            range_t = range(0, T, step)
            range_d = range(0, D, step)
            range_h = range(0, H, step)
            range_w = range(0, W, step)
        ...
        for t in range_t:
            for d in range_d:
                for h in range_h:
                    for w in range_w:
                        coords.append((t, d, h, w))
                        comp_list.append(self._get_composite_index(t, d, h, w))
    ...
    def batch_represent(self, vol_batch):
        """
        Compute array representations for a batch of 4D vector volumes efficiently.
        Supports arbitrary window size and step (linear/nonlinear) as long as all volumes
        in the batch have identical dimensions.

        Args:
            vol_batch: Tensor of shape [batch_size, T, D, H, W, m]

        Returns:
            representations: Tensor of shape [batch_size, m]
        """
        B, T, D, H, W, m = vol_batch.shape
        step = 1 if self.mode == 'linear' else self.step

        # Convert to (B, m, T, D, H, W) for unfolding
        x = vol_batch.permute(0, 5, 1, 2, 3, 4)  # [B, m, T, D, H, W]

        # Pad if necessary (for 'pad' mode)
        if self.rank_mode == 'pad':
            pad_t = (self.rank - 1) if step == 1 else (self.rank - step)
            pad_d = (self.rank - 1) if step == 1 else (self.rank - step)
            pad_h = (self.rank - 1) if step == 1 else (self.rank - step)
            pad_w = (self.rank - 1) if step == 1 else (self.rank - step)
            # Padding order: (pad_last, pad_first) for each dimension. We pad only at the end.
            x = torch.nn.functional.pad(x, (0, pad_w, 0, pad_h, 0, pad_d, 0, pad_t, 0, 0), mode='constant', value=0)
            T_pad = T + pad_t
            D_pad = D + pad_d
            H_pad = H + pad_h
            W_pad = W + pad_w
        else:
            T_pad, D_pad, H_pad, W_pad = T, D, H, W

        # Compute number of windows along each dimension
        n_windows_t = ((T_pad - self.rank) // step + 1) if T_pad >= self.rank else 0
        ...
        total_windows = n_windows_t * n_windows_d * n_windows_h * n_windows_w
        if total_windows == 0:
        ...
        # Iterate over first two dimensions (t, d) and use 2D unfold for the last two (h, w)
        win_size = self.rank * self.rank * self.rank * self.rank
        idx_offset = 0
        for t_start in range(0, T_pad - self.rank + 1, step):
            for d_start in range(0, D_pad - self.rank + 1, step):
                # Extract slice: [B, m, rank, rank, H_pad, W_pad]
                x_slice = x[:, :, t_start:t_start+self.rank, d_start:d_start+self.rank, :, :]
                # Merge first two window dimensions into channel: [B, m*rank*rank, H_pad, W_pad]
                x_merged = x_slice.contiguous().view(B, self.m * self.rank * self.rank, H_pad, W_pad)
                # Apply 2D unfold to get windows over H and W
                windows_2d = torch.nn.functional.unfold(
                    x_merged, kernel_size=(self.rank, self.rank), stride=(step, step)
                )  # [B, m*rank*rank*rank*rank, n_windows_h * n_windows_w]
        ...
        # Now compute composite indices for each window (using original dimensions, not padded)
        _, comp_indices = self._get_window_coords_and_indices(T, D, H, W)
    ...
    def batch_compute_Nk_and_targets(self, vol_batch):
        """
        Compute Nk vectors and target vectors for each position (window) in a batch of 4D volumes.
        ...
        for t_start in range(0, T_pad - self.rank + 1, step):
            for d_start in range(0, D_pad - self.rank + 1, step):
                ...
                windows_2d = torch.nn.functional.unfold(
                    x_merged, kernel_size=(self.rank, self.rank), stride=(step, step)
                )  # [B, m*rank*rank*rank*rank, n_spatial]
                windows_2d = windows_2d.view(B, self.m, win_size, -1).permute(0, 3, 2, 1)
                # Apply mapping
                mapped = torch.matmul(windows_2d, self.M.T)  # [B, n_spatial, win_size, m]
                if self.rank_op == 'sum':
        ...
        # Compute composite indices for all windows (using original dimensions)
        _, comp_indices = self._get_window_coords_and_indices(T, D, H, W)
        if len(comp_indices) == 0:
    ...
    def reg_train(self, volumes, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        Supports fast batch mode when all volumes have identical dimensions and rank_mode == 'drop'.
        ...
        """
        # Convert volumes to tensors on device
        volumes = [vol.to(self.device) if isinstance(vol, torch.Tensor) else 
                   torch.tensor(vol, dtype=torch.float32, device=self.device) for vol in volumes]
        ...
        # Check if fast batch processing is possible (all volumes same shape and rank_mode == 'drop')
        ...
        for it in range(start_iter, max_iters):
            ...
            if use_fast_batch:
                ...
                # Stack all volumes into a single tensor (they all have same shape)
                # But we need to handle batching within the loop to avoid OOM
                # We'll create batches of volumes and stack them
                for batch_start in range(0, len(indices), batch_size):
                    batch_indices = indices[batch_start:batch_start + batch_size]
                    batch_vols = [volumes[idx] for idx in batch_indices]
                    batch_targets = [t_tensors[idx] for idx in batch_indices]
                    
                    # Check if all volumes in this batch have same shape (they should, since all_equal)
                    shapes = [vol.shape for vol in batch_vols]
                    if len(set(shapes)) == 1:
                        # Stack into a single tensor
                        batch_tensor = torch.stack(batch_vols, dim=0)
                        ...
                        for vol, target in zip(batch_vols, batch_targets):
                            ...
                            optimizer.step()
                            optimizer.zero_grad()
                            total_loss += vol_loss.item()
                            total_volumes += 1
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
            else:
                # Original per-volume processing with batching
                for batch_start in range(0, len(indices), batch_size):
                    batch_indices = indices[batch_start:batch_start + batch_size]
                    batch_vols = [volumes[idx] for idx in batch_indices]
                    batch_targets = [t_tensors[idx] for idx in batch_indices]
    ...
    def cls_train(self, volumes, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        # Check if fast batch processing is possible
        all_shapes = [vol.shape for vol in volumes]
        all_equal = len(set(all_shapes)) == 1
        use_fast_batch = (self.rank_mode == 'drop' and all_equal)
            ...
            if use_fast_batch:
                for batch_start in range(0, len(indices), batch_size):
                    ...
                    if len(set(shapes)) == 1:
                        ...
                        for vol, lbl in zip(batch_vols, batch_labels):
                            vectors, comp_indices = self.extract_vectors(vol)
                            if vectors.shape[0] == 0:
                ...
                for batch_start in range(0, len(indices), batch_size):
                    ...
                    if len(set(shapes)) == 1:
                        ...
                        for vol, lbl in zip(batch_vols, batch_labels):
                            ...
                            logits = self.labeller(vol_repr.unsqueeze(0))
                            loss = criterion(logits, lbl.unsqueeze(0))
                            loss.backward()
                            optimizer.step()
                            optimizer.zero_grad()
                            ...
                            with torch.no_grad():
                                probs = torch.sigmoid(logits)
                                preds = (probs > 0.5).float()
                                total_correct += (preds == lbl.unsqueeze(0)).sum().item()
                                total_preds += lbl.numel()
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
            else:
                # Original per-volume processing with batching
    ...
    def self_train(self, volumes, max_iters=1000, tol=1e-8, learning_rate=0.01,
        ...
        if use_fast_batch:
            ...
            T, D, H, W, m = volumes[0].shape
            num_vols = len(volumes)
            all_vols = torch.stack(volumes, dim=0)  # [num_vols, T, D, H, W, m]
            vol_batch_size = batch_size  # number of volumes per batch
            ...
            for it in range(start_iter, max_iters):
                ...
                for batch_start in range(0, num_vols, vol_batch_size):
                    ...
                    batch_vols = all_vols[batch_indices]  # [batch, T, D, H, W, m]
