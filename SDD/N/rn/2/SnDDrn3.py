...
class SpatialNumDualDescriptorRN3(nn.Module):
    ...
    def extract_vectors(self, volume):
        ...
        if self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
            ...
            if D < self.rank or H < self.rank or W < self.rank:
                return torch.empty(0, self.m, device=self.device), torch.empty(0, dtype=torch.float32, device=self.device)
            
            # Permute to (1, m, D, H, W) for pooling
            vol_perm = volume.permute(3, 0, 1, 2).unsqueeze(0)  # (1, m, D, H, W)
            
            if self.rank_op == 'avg':
                # Use avg_pool3d to compute mean over each window
                pooled = F.avg_pool3d(vol_perm, kernel_size=self.rank, stride=1)
                # pooled shape: (1, m, D_win, H_win, W_win)
                D_win, H_win, W_win = pooled.shape[2], pooled.shape[3], pooled.shape[4]
                # Reshape to (num_windows, m)
                agg = pooled.squeeze(0).permute(1, 2, 3, 0).reshape(-1, self.m)  # (num_windows, m)
            else:  # sum
                # Use convolution with kernel all ones to compute sum
                kernel = torch.ones(1, self.m, self.rank, self.rank, self.rank, device=self.device)
                # kernel shape: (out_channels, in_channels, D, H, W) where out_channels = 1
                # We need to handle m channels separately: use group convolution? 
                # Simpler: use avg_pool3d then multiply by window volume
                # But since we need sum, we can use avg_pool3d and multiply by (rank^3)
                avg = F.avg_pool3d(vol_perm, kernel_size=self.rank, stride=1)
                agg = avg * (self.rank ** 3)  # (1, m, D_win, H_win, W_win)
                D_win, H_win, W_win = avg.shape[2], avg.shape[3], avg.shape[4]
                agg = agg.squeeze(0).permute(1, 2, 3, 0).reshape(-1, self.m)
            
            # Generate composite indices for all windows
            # Windows are in row-major order: depth first, then height, then width
            i_indices = torch.arange(D - self.rank + 1, device=self.device).repeat_interleave((H - self.rank + 1) * (W - self.rank + 1))
            j_indices = torch.arange(H - self.rank + 1, device=self.device).repeat_interleave(W - self.rank + 1).repeat(D - self.rank + 1)
            k_indices = torch.arange(W - self.rank + 1, device=self.device).repeat((D - self.rank + 1) * (H - self.rank + 1))
            comp_indices = self._get_composite_index(i_indices, j_indices, k_indices)
...
if __name__ == "__main__":
    ...
    print("Optimized for 3D Vector Volumes")
    print("="*60)
