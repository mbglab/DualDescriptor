...
class NumDualDescriptorRN(nn.Module):
    ...
    def __init__(self, vec_dim=4, bas_dim=50, rank=1, rank_op='avg', rank_mode='drop', 
        ...
        # Basis matrix Bbasis: L×m (learnable)
    ...
    def _get_window_data(self, vec_seq):
        ...
        Args:
            vec_seq: tensor of shape (L, m) or numpy array
        ...
        if self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
            ...
            # Prepare for unfold: shape (1, m, 1, L)
            seq_reshaped = vec_seq.unsqueeze(0).permute(0, 2, 1).unsqueeze(2)
            windows = F.unfold(seq_reshaped, kernel_size=(1, self.rank), stride=1)
            num_windows = windows.shape[2]
            windows = windows.view(1, self.m, self.rank, num_windows).squeeze(0)
        ...
        agg_tensor = self.extract_vectors(vec_seq)  # returns tensor
        if agg_tensor.shape[0] == 0:
            return torch.empty(0, self.m, device=self.device), torch.empty(0, device=self.device)
        k_positions = torch.arange(agg_tensor.shape[0], dtype=torch.float32, device=self.device)
        return agg_tensor, k_positions

    def _batch_window_data_raw(self, seq_batch):
        """
        Vectorized extraction of raw (non-mapped) aggregated window vectors for a batch of equal-length sequences.
        ...
        Returns:
            ...
            k_positions: torch.Tensor of shape [total_windows] (window indices)
        """
        B, L, m = seq_batch.shape
        if L < self.rank:
            return torch.empty(0, m, device=self.device), torch.empty(0, device=self.device)
        
        # Prepare for unfold: [B, m, 1, L]
        seq_reshaped = seq_batch.permute(0, 2, 1).unsqueeze(2)
        windows = F.unfold(seq_reshaped, kernel_size=(1, self.rank), stride=1)  # [B, m*rank, N_w]
        N_w = windows.shape[2]
        windows = windows.view(B, m, self.rank, N_w)           # [B, m, rank, N_w]
        windows = windows.permute(0, 3, 2, 1)                 # [B, N_w, rank, m]
        
        if self.rank_op == 'avg':
            agg_raw = windows.mean(dim=2)    # [B, N_w, m]
        else:  # sum
            agg_raw = windows.sum(dim=2)     # [B, N_w, m]
        ...
        return agg_raw_flat, k_positions

    def batch_window_data(self, seq_batch):
        """
        Vectorized extraction of mapped aggregated window vectors for a batch of equal-length sequences.
        Only works for linear mode and rank_op in ('avg', 'sum').
        ...
        Returns:
            agg_mapped_flat: torch.Tensor of shape [total_windows, m] (mapped by M)
            k_positions: torch.Tensor of shape [total_windows]
        """
        agg_raw_flat, k_positions = self._batch_window_data_raw(seq_batch)
        if agg_raw_flat.shape[0] == 0:
            return agg_raw_flat, k_positions
        # Apply mapping matrix M
        agg_mapped_flat = torch.matmul(agg_raw_flat, self.M.T)
        return agg_mapped_flat, k_positions
    ...
    def batch_compute_Nk(self, k_tensor, vectors):
        ...
        """
        # Apply mapping matrix M to input vectors (already done if vectors are mapped)
        # But note: in self-training, vectors are already mapped (by extract_vectors), so we might double-map.
        # To keep consistency, we will NOT reapply M here. The vectors passed are the aggregated ones (already mapped).
        # In the original batch_compute_Nk, it applied M again, which might be redundant. However, to keep behavior,
        # we will follow the original: it applies M again (since extract_vectors does not apply M).
        # So here we apply M to the input vectors.
    ...
    def batch_represent(self, seq_batch):
        ...
        and then averages over windows to obtain a fixed-size vector per sequence.
        ...
        """
        B, L, m = seq_batch.shape
        agg_mapped_flat, k_positions = self.batch_window_data(seq_batch)
        if agg_mapped_flat.shape[0] == 0:
            return torch.zeros(B, m, device=self.device)
        
        # Compute Nk for all windows (second mapping applied inside batch_compute_Nk)
        Nk_all = self.batch_compute_Nk(k_positions, agg_mapped_flat)  # [total_w, m]
        
        # Split back to sequences and average
        N_w = agg_mapped_flat.shape[0] // B
        Nk_seq = Nk_all.view(B, N_w, m)   # [B, N_w, m]
    ...
    def describe(self, seq):
        ...
        if agg_vecs.shape[0] == 0:
            return np.array([])
        N_batch = self.batch_compute_Nk(k_positions, agg_vecs)
    ...
    def D(self, seqs, t_list):
        ...
        for seq, t in zip(seqs, t_tensors):
            ...
            total_positions += agg_vecs.shape[0]
    ...
    def reg_train(self, seqs, t_list, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        """
        Train model using gradient descent with batch processing across all windows.
        Optimized version: if all sequences in a batch have equal length and conditions allow,
        uses vectorized batch_represent; otherwise falls back to per-sequence window collection.
        """
        ...
        for it in range(start_iter, max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                if len(set(seq_lengths)) == 1 and self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
                    # Equal length and conditions allow: use batch_represent
                    ...
                    # Unequal lengths or unsupported mode: fallback to per-sequence window collection
                    all_agg = []
                    all_k = []
                    seq_window_counts = []
                    ...
                    Nk_all = self.batch_compute_Nk(all_k, all_agg)
    ...
    def cls_train(self, seqs, labels, num_classes, max_iters=1000, tol=1e-8, learning_rate=0.01,
                  continued=False, decay_rate=1.0, print_every=10, batch_size=32,
                  checkpoint_file=None, checkpoint_interval=10):
        """Optimized classification training using batch_represent for equal-length sequences."""
        ...
        for it in range(max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                seq_lengths = [seq.shape[0] for seq in batch_seqs]
                if len(set(seq_lengths)) == 1 and self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
                    # Equal length: use batch_represent
                    seq_tensor = torch.stack(batch_seqs, dim=0)
                    reps = self.batch_represent(seq_tensor)
                    ...
                    # Unequal lengths: fallback to per-sequence window collection
                    all_agg = []
                    all_k = []
                    seq_window_counts = []
    ...
    def lbl_train(self, seqs, labels, num_labels, max_iters=1000, tol=1e-8, learning_rate=0.01, 
        ...
        """Optimized multi-label classification training using batch_represent for equal-length sequences."""
    ...
    def self_train(self, seqs, max_iters=1000, tol=1e-8, 
               ...
               print_every=10, batch_size=32, checkpoint_file=None, 
               checkpoint_interval=10):
        """
        Train the model using self-supervised learning with batch window processing.
        Self-training uses the gap-filling objective: predict vector mappings from position information.
        Optimized version: if all sequences in a batch have equal length and conditions allow,
        uses vectorized batch_window_data and batch_compute_Nk; otherwise falls back to per-sequence processing.
        """
        # Load checkpoint if continuing
        start_iter = 0
        best_loss = float('inf')
        ...
        for it in range(start_iter, max_iters):
            ...
            for batch_start in range(0, len(indices), batch_size):
                ...
                seq_lengths = [seq.shape[0] for seq in batch_seqs]
                if len(set(seq_lengths)) == 1 and self.mode == 'linear' and self.rank_op in ('avg', 'sum'):
                    # Equal length: use vectorized batch_window_data
                    seq_tensor = torch.stack(batch_seqs, dim=0)  # [B, L, m]
                    agg_mapped_flat, k_positions = self.batch_window_data(seq_tensor)
                    if agg_mapped_flat.shape[0] == 0:
                        continue
                    # Compute Nk for all windows (second mapping)
                    Nk_all = self.batch_compute_Nk(k_positions, agg_mapped_flat)
                    # Self-supervised loss: each window should map to its own vector after M
                    loss = torch.mean(torch.sum((Nk_all - agg_mapped_flat) ** 2, dim=1))
                    loss.backward()
                    optimizer.step()
                    
                    total_loss += loss.item() * agg_mapped_flat.size(0)
                    total_samples += agg_mapped_flat.size(0)
                else:
                    ...
                    for seq in batch_seqs:
                        ...
                        if agg_vecs.shape[0] == 0:
                            continue
                        Nk_batch = self.batch_compute_Nk(k_positions, agg_vecs)
                        loss = torch.mean(torch.sum((Nk_batch - agg_vecs) ** 2, dim=1))
                        loss.backward()
                        optimizer.step()
    ...
    def _compute_training_statistics(self, seqs, batch_size=50):
        ...
        with torch.no_grad():
            for i in range(0, len(seqs), batch_size):
                ...
                for seq in batch_seqs:
                    agg_vecs, k_positions = self._get_window_data(seq)
                    batch_token_count += agg_vecs.shape[0]
                    if agg_vecs.shape[0] > 0:
                        Nk_batch = self.batch_compute_Nk(k_positions, agg_vecs)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("NumDualDescriptorRN - PyTorch GPU Accelerated Version (Optimized)")
    print("Optimized for Numerical Vector Sequences with Equal-Length Acceleration")
    print("="*60)
